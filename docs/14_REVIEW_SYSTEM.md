...
| 行為 | 報酬 | 理由 |
|---|---|---|
| T-MANAGE内アンケート回答 | ✅ **次回1,000円OFFクーポン（併用可）** | 社内調査であり Google と無関係。全員平等に配布 |
| HP掲載許可（自社サイト） | ✅ 500pt（マイページ登録者のみ） | 自社サイトは Google 規制の対象外 |
| Google 口コミ投稿 | ❌ 一切なし | **絶対に付与しない** |

Google 投稿は「アンケート完了後のお礼として任意でご案内」するのみ。ポイントインセンティブと切り離す。

### 重要: アンケート報酬はなぜ「ポイント」ではなく「クーポン」なのか

ポイントは「マイページ登録者しか貯められない」ため、未登録のお客様にアンケート回答のお礼としてポイント付与しても使えない。**1,000円OFFクーポン方式にすることで、登録の有無に関わらず全員が平等に報酬を受け取れる** 設計になる。

クーポンの特徴：
- 次回予約時に使える「1,000円OFF」
- **既存の割引（新規クーポン、平日割引、紹介割引等）と併用可能**
- 有効期限3ヶ月（リピート促進のため）
- 1予約1アンケート、1顧客1クーポンの発行制限
- コード発行（`SV-A3F9K2` 等のランダム文字列）で受渡
...
1. マイページに通知「🌸 本日のご感想をお聞かせください（次回1,000円OFF）」
2. 顧客が通知タップ → アンケート画面へ
3. 評価項目に回答（2〜3分想定）
4. AI 生成文章の確認・編集
5. 回答確定 → `customer_surveys` に保存
6. **1,000円OFFクーポン自動発行**（`survey_coupons` に insert）
7. マイページ「マイクーポン」セクションに表示
8. 完了画面で Google 投稿誘導 + HP掲載同意オプション（同意で+500pt）

### シナリオB: 非登録者

**トリガー**: ルーム内の案内カード QR コードをお客様がスキャン
**認証**: 電話番号下4桁 + 予約日時で簡易紐付け

1. `/survey/[token]` にアクセス（トークンは予約IDから生成）
2. 「お客様のお名前 or 電話番号下4桁」で予約を特定
3. 同じアンケート画面
4. 回答確定 → `customer_surveys` に保存（`customers.id` で紐付け・電話番号から自動検索）
5. **1,000円OFFクーポン発行**（登録者と完全平等）
6. クーポンコード表示（例: `SV-A3F9K2`）+ スクショ推奨
7. 完了画面で Google 投稿誘導 + マイページ登録誘導（登録するとクーポン自動引継ぎ）
...
CREATE TABLE IF NOT EXISTS customer_surveys (
  ...
  coupon_id bigint,                         -- survey_coupons.id への参照
...
**`point_settings` テーブル**（クーポン金額＋HP掲載ボーナスの設定）:
```sql
-- 既存 review_bonus は使用しない（代わりにクーポン方式）
-- 廃止する場合は null に: UPDATE point_settings SET review_bonus = 0;
...
アンケート報酬は「**全員平等に 1,000円OFF クーポン**」、HP掲載同意は「**500pt**（登録者のみ）」、Google 投稿は「**報酬なし**（ポリシー遵守）」。

### 報酬一覧

| アクション | 報酬 | 付与タイミング | 条件 |
|---|---|---|---|
| アンケート回答完了 | 🎟 **次回1,000円OFFクーポン** | 即時発行 | 全員、1予約1回まで |
| HP掲載同意 | 🎁 **500pt** | 社内承認後 | マイページ登録者、公開承認時点 |
| Google 口コミ投稿 | ❌ **なし** | - | **絶対に付与しない**（ポリシー違反） |
| 来店によるシャドウポイント蓄積 | 裏で **正規の50%** | 施術完了時 | マイページ未登録者、登録時に2倍化 |

### クーポン仕様

**発行ルール**
- アンケート回答確定で即時発行
- コード形式: `SV-XXXXXX`（ランダム6文字、衝突チェック付き）
- 有効期限: 発行日から **3ヶ月**（リピート促進）
- 1顧客1クーポン（同時保有不可、未使用中は新規発行されない）

**併用ルール**
- ✅ 既存割引との併用可（`combinable = true`）
- ✅ 新規クーポン、平日割引、紹介割引、誕生月割引 … すべて重ねて使用可
- ✅ ポイント使用との併用可
- ❌ 他のアンケートクーポンとの重複使用は不可
...
**使用時のフロー**
1. 予約時にお客様が「アンケートクーポンあります」と申告 or コード提示
2. スタッフが T-MANAGE でコード入力 or 電話番号検索
3. `survey_coupons.used_at` をマーク、`used_reservation_id` を記録
4. 精算モーダルに自動で1,000円OFF適用（新規カラム `survey_coupon_discount` に計上）
5. 売上集計時は既存の `discount_total` に含めて扱う

### DB設計追加

```sql
-- 新規テーブル: survey_coupons
...
CREATE INDEX idx_survey_coupons_customer ON survey_coupons(customer_id);
CREATE INDEX idx_survey_coupons_code ON survey_coupons(code);
CREATE INDEX idx_survey_coupons_unused ON survey_coupons(customer_id, used_at) WHERE used_at IS NULL;
CREATE INDEX idx_survey_coupons_expires ON survey_coupons(expires_at);

ALTER TABLE survey_coupons DISABLE ROW LEVEL SECURITY;

-- reservations に精算用カラム追加
...
- HP 掲載同意にチェック → まず「承認待ち」ステータス
- 運営側（`/survey-dashboard` から）承認 → 実際にHP掲載＋500pt付与
- 承認しない場合は回答は保存されたまま、ポイント付与なし
- 既存 `point_settings.hp_publish_bonus`（500pt）を参照
- `customer_points` に `description = '🌸 HP掲載ご協力ありがとうございます'` で insert

### シャドウポイント（別仕様）

マイページ未登録者にも裏でポイントを蓄積し、登録時に2倍化して解放する仕組み。詳細は **`docs/15_SHADOW_POINTS.md`** 参照。
...
- **2026-04-25 改訂**: アンケート報酬設計の刷新
  - アンケート報酬を「1,000pt」から「**次回1,000円OFFクーポン**」に変更
  - 理由: 未登録者にもポイントが付かないと不公平、クーポンなら全員平等
  - クーポンは既存割引と**併用可**
  - `survey_coupons` テーブルを新設
  - 完了画面UIにクーポンコード表示を追加
  - 関連: シャドウポイント仕様を別文書 `15_SHADOW_POINTS.md` に分離
