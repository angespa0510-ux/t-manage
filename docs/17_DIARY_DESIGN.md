...
## 🆕 拡張仕様 (Session 66) — ストーリーズ機能 + データ保存期間管理

### 📸 ストーリーズ機能の追加

写メ日記とは別に **「24時間で消える画像/動画」** のストーリーズ機能を追加。
Instagram ストーリー風の UX で、お客様マイページから閲覧可能。

#### 写メ日記 vs ストーリーの使い分け

| 軸 | 写メ日記 | ストーリーズ |
|---|---|---|
| **公開期間** | 永続 (アーカイブされても残る) | 24時間で自動非公開 |
| **メディア** | 画像のみ (1〜10枚) | 画像 or 動画 (1つ) |
| **動画長さ** | - | 最大15秒 |
| **テキスト** | タイトル+本文(2000字) | 短いキャプション(200字) |
| **タグ** | あり (検索/フィルタ用) | なし |
| **駅ちか送信** | あり (全公開時) | **なし** (HP独自機能) |
| **コメント** | あり | なし (リアクションのみ) |
| **ハードル** | 高い (じっくり投稿) | 低い (気軽に投稿) |
...
- 長さ: 最大 **15秒**
- サイズ: 20MB まで
- 解像度: 720p (1280x720) 上限
- アップロード時にサーバー側で再エンコード推奨

**閲覧:**
- HP `/diary` のセラピストアイコン上部に「ストーリーリング」表示
- タップでフルスクリーン表示 → 5秒/枚で自動進行 (タップで送り)
- 会員ログイン時のみ「誰が見たか」をセラピストに表示

### 🗑 データ保存期間管理 (Retention Policy)

#### 全体方針

ストレージコスト最適化 + 法的観点 (退店者画像残存問題) から、データ保存期間を厳格に管理。

| データ種別 | 公開期間 | Storage実体保持 | DBレコード保持 |
|---|---|---|---|
| 写メ日記テキスト | 永続 | - | 永続 |
| 写メ日記画像 (active) | 永続 | 6ヶ月後にアーカイブ | 永続 |
| 写メ日記画像 (退店者) | 即非公開 | 30日後物理削除 | 永続 (FK保持) |
| 写メ日記画像 (休止者) | 即非公開 | 6ヶ月後物理削除 | 永続 |
| ストーリー画像/動画 | 24時間 | **24時間で物理削除** | サムネのみ30日 |
| ストーリーサムネ(50px) | - | 30日 | 30日 |

#### セラピストステータスの拡張

| status | 意味 | 写メ日記 | ストーリー | Storage削除 |
|---|---|---|---|---|
| `active` | 在籍中 | 公開 | 公開 | しない |
| `inactive` | 休止中 | 即非公開 (`unlisted`) | 即非公開 | 6ヶ月後 |
| `retired` | 退店 | 即非公開 (`unlisted`) | 即非公開 | **30日後** |
| `deleted_at` | 論理削除 | 即非公開 | 即非公開 | 30日後 |

ステータス変更時の自動処理: `handle_therapist_status_change()` トリガー
- 写メ日記の `status` を `published` → `unlisted` に変更
- 公開中ストーリーを即座に `expired` に変更
- `storage_deletion_queue` に削除予定を登録

active 復帰時:
- `unlisted` の写メ日記を `published` に戻す
- 削除キューを `cancelled` にする
- (※ 退店からの復帰は通常ないので想定外)

#### スタッフ側にもストーリーは残さない理由

**結論**: スタッフ側にも 24時間で物理削除 (サムネ50pxとログのみ残す)

理由:
1. **セラピストの心理的負担軽減** — 「監視されてる」感を排除し気軽に投稿できる UX
2. **ストレージコスト** — 動画が増えるとSupabase Pro でも厳しい
3. **代替策あり** — リアルタイム監視機能で公開中に確認可能

代わりに以下で対応:
- 📡 **ライブストーリー監視画面** (`/diary-stories-monitor`) — 公開中のストーリーを一覧表示
- 🚨 **通報機能** — お客様が問題ある投稿を報告可能 → スタッフに即通知
- 📊 **投稿ログ** — 「いつ誰が何回投稿したか」のメタデータは永続保存
  ...
  "crons": [
    {
      ...
      "schedule": "*/15 * * * *",
      "comment": "15分ごとに期限切れストーリーを処理"
    },
    {
      "path": "/api/diary/process-deletion-queue",
      "schedule": "0 3 * * *",
      "comment": "毎日3時に Storage 削除キュー実行"
    }
  ]
}
```

#### `/api/diary/cleanup-stories` の処理

1. `expires_at < NOW()` のストーリーを `status='expired'` に更新
2. `storage_deletion_queue` にメディアファイル削除予定を登録 (即時)
3. 統計用に `view_count`, `unique_viewer_count` を最終確定

#### `/api/diary/process-deletion-queue` の処理

1. `scheduled_delete_at < NOW() && status='pending'` の対象を取得
2. Supabase Storage から実体ファイルを削除
3. `status='completed'` に更新 + `completed_at` 記録
4. 失敗したら `attempted_count++` + `last_error` 記録 (3回失敗で `failed`)

### 📋 実装すべき API (Phase 1.5)

| エンドポイント | 用途 |
|---|---|
| `POST /api/diary/story/post` | ストーリー投稿 |
| `GET /api/diary/story/active` | 現在公開中のストーリー一覧 (HP用) |
| `GET /api/diary/story/[id]` | 個別ストーリー (閲覧+カウント) |
| `POST /api/diary/story/[id]/view` | 閲覧ログ記録 (会員のみ) |
| `POST /api/diary/story/[id]/react` | リアクション (Phase 2) |
| `DELETE /api/diary/story/[id]` | セラピスト本人 or スタッフ削除 |
| `POST /api/diary/report` | 通報機能 (story/entry/comment 共通) |
| `GET /api/diary/cleanup-stories` | Cron用: 期限切れ処理 |
| `GET /api/diary/process-deletion-queue` | Cron用: Storage実体削除 |
| `GET /api/diary/stories/views/[storyId]` | セラピスト用: 誰が見たか |
...
| データ | 件数 | 平均サイズ | 月間容量 |
|---|---|---|---|
| 写メ日記画像 | 100人×月25投稿×4枚=10,000枚 | 300KB (WebP) | 3GB |
| ストーリー画像 (24時間で削除) | 100人×月50投稿=5,000枚 | 500KB | 2.5GB → ほぼ0 |
| ストーリー動画 (24時間で削除) | 100人×月10動画=1,000本 | 10MB | 10GB → ほぼ0 |
| ストーリーサムネ (30日保持) | 6,000枚 | 5KB | 30MB |
| **永続保持の純増** | - | - | **約 3GB/月** |

→ 1年後でも 36GB 程度。Supabase Pro (100GB) 内に収まる。
半年経過分の写メ日記画像をアーカイブ圧縮すれば、さらに長期運用可能。
