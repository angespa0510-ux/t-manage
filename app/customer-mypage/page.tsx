...
type Customer = { id: number; name: string; phone: string; phone2: string; phone3: string; email: string; notes: string; rank: string; login_email: string; login_password: string; created_at: string };
...
export default function CustomerMyPage() {
  ...
  const [setEmail, setSetEmail] = useState(""); const [setPw, setSetPw] = useState(""); const [setPwConfirm, setSetPwConfirm] = useState(""); const [settingMsg, setSettingMsg] = useState(""); const [settingSaving, setSettingSaving] = useState(false);
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: p } = await supabase.from("customer_points").select("*").eq("customer_id", customer.id).order("created_at", { ascending: false }); if (p) setPoints(p);
  ...
  const submitBooking = async () => {
    ...
    if (bookPointUse > 0) { await supabase.from("customer_points").insert({ customer_id: customer.id, amount: -bookPointUse, type: "use", description: `予約利用 ${dateFmt(bookDate)}` }); }
  ...
  return (<div className="min-h-screen pb-20" style={{ backgroundColor: C.bg, color: C.text }}>
    ...
    {/* ポイント履歴モーダル */}
    {showPoints && (<div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-end sm:items-center justify-center z-50" onClick={() => setShowPoints(false)}><div className="rounded-t-2xl sm:rounded-2xl border w-full max-w-md max-h-[80vh] overflow-y-auto animate-[slideUp_0.3s]" style={{ backgroundColor: C.card, borderColor: C.border }} onClick={e => e.stopPropagation()}><div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${C.border}` }}><h3 className="text-[16px] font-medium">🎁 ポイント履歴</h3><button onClick={() => setShowPoints(false)} className="text-[14px] cursor-pointer p-1" style={{ color: C.textMuted }}>✕</button></div><div className="px-6 py-4"><div className="text-center mb-4"><p className="text-[11px]" style={{ color: C.textMuted }}>ポイント残高</p><p className="text-[28px] font-bold" style={{ color: C.accent }}>{pointBalance.toLocaleString()}<span className="text-[12px] font-normal ml-1">pt</span></p></div>{points.length === 0 ? (<p className="text-center py-4 text-[12px]" style={{ color: C.textFaint }}>ポイント履歴がありません</p>) : (<div className="space-y-2">{points.map(p => (<div key={p.id} className="rounded-xl border p-3 flex items-center justify-between" style={{ borderColor: C.border }}><div><p className="text-[12px] font-medium">{p.description || (p.type === "earn" ? "ポイント付与" : "ポイント利用")}</p><span className="text-[10px]" style={{ color: C.textMuted }}>{new Date(p.created_at).toLocaleDateString("ja-JP")}</span></div><span className="text-[14px] font-bold" style={{ color: p.amount > 0 ? C.green : C.red }}>{p.amount > 0 ? "+" : ""}{p.amount.toLocaleString()}pt</span></div>))}</div>)}</div></div></div>)}
