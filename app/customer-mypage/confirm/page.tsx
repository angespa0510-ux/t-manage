...
type Customer = { id: number; name: string; phone: string; phone2: string; phone3: string; email: string; notes: string; rank: string; login_email: string; login_password: string; created_at: string; birthday: string };
...
export default function CustomerMypage() {
  ...
  const [setEmail, setSetEmail] = useState(""); const [setPw, setSetPw] = useState(""); const [setPwConfirm, setSetPwConfirm] = useState(""); const [settingMsg, setSettingMsg] = useState(""); const [settingSaving, setSettingSaving] = useState(false); const [setBday, setSetBday] = useState("");
  ...
  useEffect(() => { const saved = localStorage.getItem("customer_mypage_id"); if (saved) { supabase.from("customers").select("*").eq("id", Number(saved)).single().then(({ data }) => { if (data) { setCustomer(data); setSetEmail(data.login_email || ""); setSetBday(data.birthday || ""); } }); } }, []);
  ...
  const saveSettings = async () => { if (!customer) return; setSettingMsg(""); setSettingSaving(true); const updates: Record<string, string | null> = {}; if (setEmail.trim() && setEmail.trim() !== customer.login_email) updates.login_email = setEmail.trim(); if ((setBday || "") !== (customer.birthday || "")) updates.birthday = setBday || null; if (setPw) { if (setPw.length < 6) { setSettingMsg("パスワードは6文字以上にしてください"); setSettingSaving(false); return; } if (setPw !== setPwConfirm) { setSettingMsg("パスワードが一致しません"); setSettingSaving(false); return; } updates.login_password = setPw; } if (Object.keys(updates).length === 0) { setSettingMsg("変更がありません"); setSettingSaving(false); return; } const { error } = await supabase.from("customers").update(updates).eq("id", customer.id); setSettingSaving(false); if (error) setSettingMsg("保存に失敗しました"); else { setSettingMsg("保存しました！"); setSetPw(""); setSetPwConfirm(""); const { data } = await supabase.from("customers").select("*").eq("id", customer.id).single(); if (data) { setCustomer(data); setSetBday(data.birthday || ""); } setTimeout(() => setSettingMsg(""), 2000); } };
