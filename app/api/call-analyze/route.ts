...
const SYSTEM_PROMPT = `あなたはメンズエステサロン「アンジュスパ」の通話分析アシスタントです。
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1回目: Sonnet 4.6（force_opus=true なら最初から Opus）
    const primaryModel = forceOpus ? MODEL_OPUS : MODEL_SONNET;
    const first = await callClaude(primaryModel, transcript);
    ...
    // エスカレーション判定（Sonnet が should_escalate=true と言ったら Opus 再実行）
    if (!forceOpus && first.result.should_escalate) {
      try {
        const second = await callClaude(MODEL_OPUS, transcript);
        ...
        escalated = true;
        escalationReason = first.result.escalation_reason || "Sonnet判断";
      } catch (e) {
