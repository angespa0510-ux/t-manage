...
function ConfirmContent() {
  ...
  useEffect(() => {
    const verify = async () => {
      ...
      const { data } = await supabase.from("staff").select("id, name, email_verified").eq("email_token", token).maybeSingle();
      if (!data) { setStatus("error"); return; }
      if (data.email_verified) { setName(data.name); setStatus("already"); return; }
      await supabase.from("staff").update({ email_verified: true }).eq("id", data.id);
      setName(data.name);
      setStatus("success");
    };
    verify();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#faf8f5] p-4">
      <div className="w-full max-w-sm rounded-2xl bg-white shadow-lg p-8 text-center">
        ...
        {status === "success" && <><h1 className="text-[18px] font-medium mb-2 text-[#333]">確認完了！</h1><p className="text-[13px] text-[#888]">{name} さんのメールアドレスが確認されました</p><p className="text-[12px] mt-4 text-[#aaa]">このページを閉じてください</p></>}
        {status === "already" && <><h1 className="text-[18px] font-medium mb-2 text-[#333]">確認済みです</h1><p className="text-[13px] text-[#888]">{name} さんのメールアドレスは既に確認されています</p></>}
...
export default function ConfirmStaffEmailPage() {
