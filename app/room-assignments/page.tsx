                ...
                {isExp && (
                  <div className="px-5 pb-5 animate-[fadeIn_0.3s]" style={{ backgroundColor: T.card }}>
                    <div className="flex items-center gap-2 mb-4 pt-2">
                      <button onClick={() => autoAssign(date)} className="px-3 py-1.5 text-white text-[10px] rounded-lg cursor-pointer" style={{ backgroundColor: T.accent }}>シフトから自動割当</button>
                    </div>
                            ...
                            return (<div key={room.id} className="rounded-xl p-2 space-y-1.5" style={{ backgroundColor: T.cardAlt }}>
                              <div className="flex items-stretch gap-2">
                                ...
                                <div className="flex-1 rounded-lg p-2" style={{ border: `2px solid ${eB}30`, backgroundColor: eA ? eB + "08" : T.card }}>
                                  ...
                                  {eA && (<>
                                    <div className="flex items-center gap-1 mt-1"><select value={eA.start_time} onChange={(e) => updateTime(eA.id, "start_time", e.target.value)} className="px-1 py-0.5 rounded text-[9px] outline-none cursor-pointer border" style={{ backgroundColor: T.card, borderColor: T.border, color: T.textSub }}>{TIMES.map((t) => (<option key={t} value={t}>{t}</option>))}</select><span className="text-[9px]" style={{ color: T.textFaint }}>〜</span><select value={eA.end_time} onChange={(e) => updateTime(eA.id, "end_time", e.target.value)} className="px-1 py-0.5 rounded text-[9px] outline-none cursor-pointer border" style={{ backgroundColor: T.card, borderColor: T.border, color: T.textSub }}>{TIMES.map((t) => (<option key={t} value={t}>{t}</option>))}</select></div>
                                    {bTS.length > 0 && (<div className="flex items-center gap-1.5 mt-1"><span className="text-[8px]" style={{ color: "#85a8c4" }}>🅿</span><select value={eP?.parking_spot_id || 0} onChange={(e) => assignParking(date, eA.therapist_id, Number(e.target.value))} className="px-1.5 py-0.5 rounded text-[9px] outline-none cursor-pointer border" style={{ borderColor: eP ? "#85a8c466" : T.border, backgroundColor: eP ? "#85a8c412" : T.card, color: eP ? "#3d6b9f" : T.textMuted, fontWeight: eP ? 600 : 400 }}><option value={0}>なし</option>{bTS.map((sp) => { const u = isSpotUsedOnDate(date, sp.id, eA.therapist_id); return (<option key={sp.id} value={sp.id} disabled={u}>{sp.number}{u ? "（使用中）" : ""}</option>); })}</select></div>)}
                                    <div className="flex items-center gap-1.5 mt-1"><CleanSel value={eClean} onChange={(v) => updateCleaning(eA.id, v)} ph="早番清掃" /></div>
                                    <div className="flex items-center gap-1 mt-1.5"><AttBtn label="当欠" color="#c45555" active={false} onClick={() => markAbsent(date, eA)} /><AttBtn label="遅刻" color="#f59e0b" active={eA.attendance?.includes("late") || false} onClick={() => toggleAttendance(eA.id, eA.attendance || "", "late")} /><AttBtn label="早退" color="#a78bfa" active={eA.attendance?.includes("early_leave") || false} onClick={() => toggleAttendance(eA.id, eA.attendance || "", "early_leave")} /></div>
                                    <input type="text" placeholder="早番メモ" defaultValue={eA.memo || ""} onBlur={(e) => { if (e.target.value !== (eA.memo || "")) updateSlotMemo(eA.id, e.target.value); }} onKeyDown={(e) => { if (e.key === "Enter") (e.target as HTMLInputElement).blur(); }} className="w-full mt-1.5 px-2 py-1 rounded text-[9px] outline-none border" style={{ backgroundColor: T.card, borderColor: T.border, color: T.text }} />
                                  </>)}
                                </div>
                                {/* Late */}
                                <div className="flex-1 rounded-lg p-2" style={{ border: `2px solid ${lB}30`, backgroundColor: lA ? lB + "08" : T.card }}>
                                  ...
                                  {lA && (<>
                                    <div className="flex items-center gap-1 mt-1"><select value={lA.start_time} onChange={(e) => updateTime(lA.id, "start_time", e.target.value)} className="px-1 py-0.5 rounded text-[9px] outline-none cursor-pointer border" style={{ backgroundColor: T.card, borderColor: T.border, color: T.textSub }}>{TIMES.map((t) => (<option key={t} value={t}>{t}</option>))}</select><span className="text-[9px]" style={{ color: T.textFaint }}>〜</span><select value={lA.end_time} onChange={(e) => updateTime(lA.id, "end_time", e.target.value)} className="px-1 py-0.5 rounded text-[9px] outline-none cursor-pointer border" style={{ backgroundColor: T.card, borderColor: T.border, color: T.textSub }}>{TIMES.map((t) => (<option key={t} value={t}>{t}</option>))}</select></div>
                                    {bTS.length > 0 && (<div className="flex items-center gap-1.5 mt-1"><span className="text-[8px]" style={{ color: "#85a8c4" }}>🅿</span><select value={lP?.parking_spot_id || 0} onChange={(e) => assignParking(date, lA.therapist_id, Number(e.target.value))} className="px-1.5 py-0.5 rounded text-[9px] outline-none cursor-pointer border" style={{ borderColor: lP ? "#85a8c466" : T.border, backgroundColor: lP ? "#85a8c412" : T.card, color: lP ? "#3d6b9f" : T.textMuted, fontWeight: lP ? 600 : 400 }}><option value={0}>なし</option>{bTS.map((sp) => { const u = isSpotUsedOnDate(date, sp.id, lA.therapist_id); return (<option key={sp.id} value={sp.id} disabled={u}>{sp.number}{u ? "（使用中）" : ""}</option>); })}</select></div>)}
                                    <div className="flex items-center gap-1.5 mt-1"><CleanSel value={lA.cleaning || ""} onChange={(v) => updateCleaning(lA.id, v)} ph="遅番清掃" /></div>
                                    <div className="flex items-center gap-1 mt-1.5"><AttBtn label="当欠" color="#c45555" active={false} onClick={() => markAbsent(date, lA)} /><AttBtn label="遅刻" color="#f59e0b" active={lA.attendance?.includes("late") || false} onClick={() => toggleAttendance(lA.id, lA.attendance || "", "late")} /><AttBtn label="早退" color="#a78bfa" active={lA.attendance?.includes("early_leave") || false} onClick={() => toggleAttendance(lA.id, lA.attendance || "", "early_leave")} /></div>
                                    <input type="text" placeholder="遅番メモ" defaultValue={lA.memo || ""} onBlur={(e) => { if (e.target.value !== (lA.memo || "")) updateSlotMemo(lA.id, e.target.value); }} onKeyDown={(e) => { if (e.key === "Enter") (e.target as HTMLInputElement).blur(); }} className="w-full mt-1.5 px-2 py-1 rounded text-[9px] outline-none border" style={{ backgroundColor: T.card, borderColor: T.border, color: T.text }} />
                                  </>)}
                                </div>
                              </div>
                              <div className="flex items-center gap-2 pl-[58px]">
                                <button onClick={() => updateDailySetting(date, room.id, { send_off: !so })} className="text-[9px] px-2 py-1 rounded-md cursor-pointer border font-medium" style={{ borderColor: so ? "#fbbf2466" : T.border, backgroundColor: so ? "#fbbf2418" : "transparent", color: so ? "#b45309" : T.textMuted }}>送{so ? "有" : "無"}</button>
                              </div>
                            </div>);
                          })}</div>
                        </div>);
