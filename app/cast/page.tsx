                ...
                {m.content ? (
                  <div style={{ fontFamily: FONT_SERIF, color: T.text }}>
                    {m.content.split("\n").map((line, i) => {
                      ...
                      // 画像構文 ![alt](url) — SVG図解 / Gemini画像 / Supabase Storage 画像
                      if (line.startsWith("![")) { const im = line.match(/!\[(.*?)\]\((.*?)\)/); if (im) return <img key={i} src={im[2]} alt={im[1]} style={{ display: "block", margin: "12px auto", maxWidth: "100%", height: "auto" }} />; }
                      // YouTube 埋め込み [youtube:VIDEO_ID]
                      if (line.match(/^\[youtube:([\w-]+)\]$/)) { const vid = line.match(/^\[youtube:([\w-]+)\]$/)?.[1]; return <div key={i} style={{ position: "relative", paddingBottom: "56.25%", height: 0, margin: "12px 0", overflow: "hidden" }}><iframe src={`https://www.youtube.com/embed/${vid}`} style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", border: "none" }} allowFullScreen /></div>; }
                      // Google Drive 動画埋め込み [gdrive:FILE_ID:説明]
                      if (line.match(/^\[gdrive:([\w-]+)(:.*)?\]$/)) { const gm = line.match(/^\[gdrive:([\w-]+)(?::(.+))?\]$/); const fid = gm?.[1]; const gdesc = gm?.[2] || ""; return <div key={i} style={{ margin: "14px 0" }}><div style={{ position: "relative", paddingBottom: "56.25%", height: 0, overflow: "hidden" }}><iframe src={`https://drive.google.com/file/d/${fid}/preview`} style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", border: "none" }} allow="autoplay" /></div>{gdesc && <p style={{ fontSize: 11, fontWeight: 500, marginTop: 6, textAlign: "center", color: T.accent, fontFamily: FONT_SERIF, letterSpacing: "0.05em" }}>🎬 {gdesc}</p>}</div>; }
