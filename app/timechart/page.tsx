...
type Therapist = {
  ...
  deleted_at?: string | null;      // 論理削除日時（NULL = 未削除）
};
...
type TcConfig = {
  ...
  // 超過通知設定
  alertStartEnabled: boolean; alertStartMin: number; // 来店遅れ（予約時間後○分）
  alertEndEnabled: boolean; alertEndMin: number; // 終了遅れ（終了時間後○分）
  alertSummaryEnabled: boolean; alertSummaryMin: number; // 概要送信済既読（予約○分前）
  alertDetailEnabled: boolean; alertDetailMin: number; // 詳細送信済既読（予約○分前）
  alertPhoneEnabled: boolean; alertPhoneMin: number; // 電話確認（予約○分前）
};
...
function TimeChartInner() {
  ...
  const [tcConfig, setTcConfig] = useState<TcConfig>({
    ...
    alertStartEnabled: true, alertStartMin: 5,
    alertEndEnabled: true, alertEndMin: 5,
    alertSummaryEnabled: false, alertSummaryMin: 30,
    alertDetailEnabled: false, alertDetailMin: 30,
    alertPhoneEnabled: true, alertPhoneMin: 60,
  });
  // Cancel states
  ...
  // アンケートクーポン自動適用
  const [editSurveyCouponId, setEditSurveyCouponId] = useState<number | null>(null);
  const [editSurveyCouponCode, setEditSurveyCouponCode] = useState<string>("");
  ...
  const [editPhoneConfirm, setEditPhoneConfirm] = useState(false);
  ...
  // 顧客ランク
  const CUST_RANKS: Record<string, { label: string; color: string; bg: string }> = {
  ...
  const [newCustRank, setNewCustRank] = useState("normal");
  const [editCustRank, setEditCustRank] = useState("normal");

  // NG数による自動ランク判定（休止・退店セラピスト除外）
  const autoCalcRank = async (customerName: string): Promise<string | null> => {
    const { data: notes } = await supabase.from("therapist_customer_notes").select("therapist_id, is_ng").eq("customer_name", customerName).eq("is_ng", true);
    if (!notes || notes.length === 0) return null;
    const { data: activeTherapists } = await supabase.from("therapists").select("id").eq("status", "active");
    const activeIds = new Set((activeTherapists || []).map(t => t.id));
    const activeNgCount = notes.filter(n => activeIds.has(n.therapist_id)).length;
    if (activeNgCount >= 5) return "banned";
    if (activeNgCount >= 3) return "caution";
    return null;
  };

  // NG警告ポップアップ
  const [ngWarning, setNgWarning] = useState<{ therapistName: string; customerName: string; ngReason: string; custRank: string } | null>(null);
  const [ngTherapistIdsForCust, setNgTherapistIdsForCust] = useState<Set<number>>(new Set());
  const fetchNgForCustomer = async (custName: string) => {
    if (!custName) { setNgTherapistIdsForCust(new Set()); return; }
    const cleanName = custName.replace(/\s*L$/i, "").trim();
    const { data } = await supabase.from("therapist_customer_notes").select("therapist_id").eq("customer_name", cleanName).eq("is_ng", true);
    setNgTherapistIdsForCust(new Set((data || []).map(d => d.therapist_id)));
  };
  ...
  type NotifyInfo = { resId?: number; custName: string; custPhone: string; custEmail: string; hasLine: boolean; isMember: boolean; date: string; startTime: string; endTime: string; course: string; therapistName: string; therapistId?: number; total: number; nomination: string; discountName: string; extensionName: string; storeName: string; buildingName: string };
  ...
  const [bulkActiveSend, setBulkActiveSend] = useState<BulkTherapistData | null>(null);
  ...
  // セラピストメモ（お客様が書いたメモ）
  type TherapistMemo = { id: number; customer_id: number; therapist_id: number; reservation_id: number; rating: number; memo: string; created_at: string; updated_at: string; customer_name?: string; res_date?: string; res_course?: string };
  const [showThMemos, setShowThMemos] = useState(false);
  const [thMemos, setThMemos] = useState<TherapistMemo[]>([]);
  const [thMemoSearch, setThMemoSearch] = useState("");

  const openThMemos = async () => {
    setShowThMemos(true);
    setThMemoSearch("");
    const { data: memos } = await supabase.from("customer_therapist_memos").select("*").order("created_at", { ascending: false }).limit(300);
    if (memos && memos.length > 0) {
      const custIds = [...new Set(memos.map((m: TherapistMemo) => m.customer_id))];
      const resIds = [...new Set(memos.map((m: TherapistMemo) => m.reservation_id))];
      const { data: custs } = await supabase.from("customers").select("id,name").in("id", custIds);
      const { data: ress } = await supabase.from("reservations").select("id,date,course").in("id", resIds);
      const custMap = new Map((custs || []).map((c: { id: number; name: string }) => [c.id, c.name]));
      const resMap = new Map((ress || []).map((r: { id: number; date: string; course: string }) => [r.id, r]));
      setThMemos(memos.map((m: TherapistMemo) => ({ ...m, customer_name: custMap.get(m.customer_id) || "不明", res_date: resMap.get(m.reservation_id)?.date || "", res_course: resMap.get(m.reservation_id)?.course || "" })));
    } else { setThMemos([]); }
  };
  ...
  // 超過通知（手動LINE送信）
  const [overdueAlerts, setOverdueAlerts] = useState<{id:number;type:"start"|"end"|"summary"|"detail"|"phone";customerName:string;therapistName:string;time:string}[]>([]);
  ...
  useEffect(() => {
    const check = () => {
      ...
      const adjNowH = nowH < 9 ? nowH + 24 : nowH;
      ...
      for (const r of reservations) {
        ...
        const adjSh = sh < 9 ? sh + 24 : sh;
        const startMin = adjSh * 60 + sm;
        const [eh, em] = (r.end_time || "13:00").split(":").map(Number);
        const adjEh = eh < 9 ? eh + 24 : eh;
        const endMin = adjEh * 60 + em;
        const tName = therapists.find(t => t.id === r.therapist_id)?.name || "";
        if (tcConfig.alertStartEnabled && nowMin >= startMin + tcConfig.alertStartMin && cs !== "serving" && cs !== "completed" && !overdueDismissedRef.current.has(`${r.id}-start`)) {
          alerts.push({ id: r.id, type: "start", customerName: r.customer_name, therapistName: tName, time: r.start_time });
        }
        if (tcConfig.alertEndEnabled && nowMin >= endMin + tcConfig.alertEndMin && cs !== "completed" && cs === "serving" && !overdueDismissedRef.current.has(`${r.id}-end`)) {
          alerts.push({ id: r.id, type: "end", customerName: r.customer_name, therapistName: tName, time: r.end_time });
        }
        if (tcConfig.alertSummaryEnabled && nowMin >= startMin - tcConfig.alertSummaryMin && nowMin < startMin && (cs === "summary_unread" || cs === "summary_read") && !overdueDismissedRef.current.has(`${r.id}-summary`)) {
          alerts.push({ id: r.id, type: "summary", customerName: r.customer_name, therapistName: tName, time: r.start_time });
        }
        if (tcConfig.alertDetailEnabled && nowMin >= startMin - tcConfig.alertDetailMin && nowMin < startMin && (cs === "detail_unread" || cs === "detail_read") && !overdueDismissedRef.current.has(`${r.id}-detail`)) {
          alerts.push({ id: r.id, type: "detail", customerName: r.customer_name, therapistName: tName, time: r.start_time });
        }
        if (tcConfig.alertPhoneEnabled && (r as any).phone_confirm && nowMin >= startMin - tcConfig.alertPhoneMin && nowMin < startMin && cs !== "completed" && !overdueDismissedRef.current.has(`${r.id}-phone`)) {
          alerts.push({ id: r.id, type: "phone", customerName: r.customer_name, therapistName: tName, time: r.start_time });
        }
      }
      if (alerts.length > 0 && alerts.length !== overdueAlerts.length) {
        setOverdueAlerts(alerts);
        setShowOverduePopup(true);
        try { const ctx = new AudioContext(); const osc = ctx.createOscillator(); const gain = ctx.createGain(); osc.connect(gain); gain.connect(ctx.destination); osc.frequency.value = 880; gain.gain.value = 0.3; osc.start(); setTimeout(() => { osc.stop(); ctx.close(); }, 500); } catch {}
      }
  ...
  }, [reservations, therapists, selectedDate, overdueAlerts.length, tcConfig]);
  ...
  const handlePanEnd = () => { if (!isPanning) return; setIsPanning(false); const c = timelineRef.current; if (!c) return; let v = velocity.current * 15; const dec = () => { if (Math.abs(v) < 0.5) return; c.scrollLeft -= v; v *= 0.92; animFrame.current = requestAnimationFrame(dec); }; dec(); setTimeout(() => { panMoved.current = false; }, 100); };
  ...
  const openEdit = (r: Reservation) => { setEditRes(r); setEditCustName(r.customer_name); setEditTherapistId(r.therapist_id); setEditStart(r.start_time); setEditEnd(r.end_time); setEditNotes(r.notes || ""); const c = courses.find((x) => x.name === r.course); setEditCourseId(c ? c.id : 0); setEditMsg(""); setEditNomination((r as any).nomination || ""); setEditNomFee((r as any).nomination_fee || 0); const discs = (r as any).discount_name ? (r as any).discount_name.split(",").map((n: string) => { const d = discounts.find(x=>x.name===n); /* アンケートクーポンは1000円OFFハードコード */ if (n === "アンケートクーポン") return { name: n, amount: 1000 }; return { name: n, amount: d ? (d.type==="percent" ? Math.round((courses.find(x=>x.name===r.course)?.price || 0) * d.amount / 100) : d.amount) : 0 }; }).filter((d: any)=>d.name) : []; setEditDiscounts(discs); setEditExtension((r as any).extension_name || ""); setEditExtPrice((r as any).extension_price || 0); setEditExtDur((r as any).extension_duration || 0); const opts = (r as any).options_text ? (r as any).options_text.split(",").map((n: string) => { const o = options.find(x=>x.name===n); return { name: n, price: o?.price || 0 }; }).filter((o: any)=>o.name) : []; setEditOptions(opts); setEditStatus((r as any).status || "unprocessed"); setEditCustomerStatus((r as any).customer_status || "unsent"); setEditTherapistStatus((r as any).therapist_status || "unsent"); setEditCardBase(String((r as any).card_base || "")); setEditPaypay(String((r as any).paypay_amount || "")); setEditStaffName((r as any).staff_name || ""); setEditPhoneConfirm((r as any).phone_confirm || false);
    // 顧客情報 + 未使用アンケートクーポン自動取得（90分以上のコース限定）
    setEditSurveyCouponId(null); setEditSurveyCouponCode("");
    supabase.from("customers").select("id,phone,rank").eq("name", r.customer_name).maybeSingle().then(async ({ data }) => {
      setEditCustPhone(data?.phone || "");
      setEditCustRank(data?.rank || "normal");
      // 既に discount_name に「アンケートクーポン」がある（過去に既適用 or 既保存）→ 今回は新規追加しない
      const alreadyApplied = ((r as any).discount_name || "").includes("アンケートクーポン");
      // この予約自体が既にクーポンを使用済みなら、その coupon_id を保持しておく
      const existingCouponId = (r as any).survey_coupon_id;
      if (existingCouponId) setEditSurveyCouponId(existingCouponId);
      // コース duration を判定（90分以上のみ自動適用対象）
      const courseDuration = c?.duration || 0;
      if (data?.id && !alreadyApplied && (r as any).customer_status !== "completed" && courseDuration >= 90) {
        const { data: coupon } = await supabase
          ...
          .maybeSingle();
        if (coupon) {
          setEditSurveyCouponId(coupon.id);
          setEditSurveyCouponCode(coupon.code);
          // 1予約に1枚のみなので重複追加しない
          setEditDiscounts(prev => prev.some(d => d.name === "アンケートクーポン") ? prev : [...prev, { name: "アンケートクーポン", amount: coupon.discount_amount || 1000 }]);
        }
      }
    });
    fetchNgForCustomer(r.customer_name); checkNgWarning(r.customer_name, r.therapist_id); fetchCancelHistory(r.customer_name); };
  // ===== ポイント自動付与（completed時） =====
  ...
  const updateReservation = async () => { if (!editRes) return; setEditSaving(true); setEditMsg(""); const eOptText = editOptions.map(o=>o.name).join(","); const eOptTotal = editOptions.reduce((s,o)=>s+o.price,0); const eCp = editSelectedCourse?.price || 0; const eDiscText = editDiscounts.map(d=>d.name).join(","); const eDiscTotal = editDiscounts.reduce((s,d)=>s+d.amount,0); const eTotal = eCp + editNomFee + eOptTotal + editExtPrice - eDiscTotal; const derivedStatus = editCustomerStatus === "completed" ? "completed" : editCustomerStatus === "serving" ? "serving" : editCustomerStatus === "web_reservation" ? "web_reservation" : editCustomerStatus.startsWith("detail") ? "processed" : editCustomerStatus.startsWith("summary") ? "email_sent" : "unprocessed"; /* アンケートクーポン適用判定 */ const usedSurveyCoupon = editDiscounts.some(d => d.name === "アンケートクーポン"); const surveyCouponDiscountAmt = editDiscounts.find(d => d.name === "アンケートクーポン")?.amount || 0; const { error } = await supabase.from("reservations").update({ customer_name: editCustName.trim(), therapist_id: editTherapistId, start_time: editStart, end_time: editEnd, course: editSelectedCourse?.name || editRes.course, notes: editNotes.trim(), nomination: editNomination, nomination_fee: editNomFee, options_text: eOptText, options_total: eOptTotal, discount_name: eDiscText, discount_amount: eDiscTotal, extension_name: editExtension, extension_price: editExtPrice, extension_duration: editExtDur, total_price: eTotal, status: derivedStatus, customer_status: editCustomerStatus, therapist_status: editTherapistStatus, card_base: parseInt(editCardBase) || 0, paypay_amount: parseInt(editPaypay) || 0, card_billing: Math.round((parseInt(editCardBase) || 0) * 1.1), cash_amount: eTotal - (parseInt(editCardBase) || 0) - (parseInt(editPaypay) || 0), staff_name: editStaffName, phone_confirm: editPhoneConfirm, survey_coupon_id: usedSurveyCoupon ? editSurveyCouponId : null, survey_coupon_discount: usedSurveyCoupon ? surveyCouponDiscountAmt : 0 }).eq("id", editRes.id); setEditSaving(false); if (error) { toast.show("更新失敗: " + error.message, "error"); } else { if (editCustomerStatus === "completed") { await awardPoints(editRes.id, editCustName.trim(), eTotal, editRes.date); /* アンケートクーポン消化: completed かつ 適用済み かつ まだ used_at が無い場合 */ if (usedSurveyCoupon && editSurveyCouponId) { const { data: existingCoupon } = await supabase.from("survey_coupons").select("used_at").eq("id", editSurveyCouponId).maybeSingle(); if (existingCoupon && !existingCoupon.used_at) { await supabase.from("survey_coupons").update({ used_at: new Date().toISOString(), used_reservation_id: editRes.id }).eq("id", editSurveyCouponId); } } } if (editManualPoints && parseInt(editManualPoints) !== 0) { const mp = parseInt(editManualPoints); const { data: cust } = await supabase.from("customers").select("id").eq("name", editCustName.trim()).maybeSingle(); if (cust) { const expAt = new Date(); expAt.setMonth(expAt.getMonth() + (ptSettings?.expiry_months || 12)); await supabase.from("customer_points").insert({ customer_id: cust.id, amount: mp, type: mp > 0 ? "earn" : "use", description: editManualPointDesc.trim() || (mp > 0 ? "手動ポイント付与" : "手動ポイント減算"), expires_at: mp > 0 ? expAt.toISOString() : null }); } setEditManualPoints(""); setEditManualPointDesc(""); } toast.show("更新しました！", "success"); /* 顧客情報も更新（レコードがなければ自動作成） */ const origName = editRes.customer_name; const { data: custRec } = await supabase.from("customers").select("id").eq("name", origName).maybeSingle(); if (custRec) { const custUpdate: Record<string, string> = { rank: editCustRank }; if (editCustPhone.trim()) custUpdate.phone = editCustPhone.trim().replace(/[-\s　()（）]/g, ""); if (editCustName.trim() !== origName) custUpdate.name = editCustName.trim(); await supabase.from("customers").update(custUpdate).eq("id", custRec.id); } else { const newCust: Record<string, string> = { name: editCustName.trim(), rank: editCustRank }; if (editCustPhone.trim()) newCust.phone = editCustPhone.trim().replace(/[-\s　()（）]/g, ""); await supabase.from("customers").insert(newCust); } /* NG自動ランク判定（手動で出禁にした場合はそちらを優先） */ if (editCustRank !== "banned") { const autoRank = await autoCalcRank(editCustName.trim()); if (autoRank) { const { data: cust3 } = await supabase.from("customers").select("id").eq("name", editCustName.trim()).maybeSingle(); if (cust3) await supabase.from("customers").update({ rank: autoRank }).eq("id", cust3.id); } } fetchData(); setTimeout(() => { setEditRes(null); setEditMsg(""); }, 600); } };
  ...
  const openBulkNotify = async () => {
    ...
    for (const [tidStr, rsvs] of Object.entries(grouped)) {
      ...
      rsvs.forEach((r, i) => {
        ...
        const custLink = customerMypageUrl(cleanName);
    ...
    <div className="h-screen flex flex-col" style={{ backgroundColor: T.bg, color: T.text }}>
      ...
      <div className="h-[64px] backdrop-blur-xl border-b flex items-center justify-between px-6 flex-shrink-0" style={{ backgroundColor: dark ? T.card + "cc" : "rgba(255,255,255,0.8)", borderColor: T.border }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowOverduePopup(true)} className="relative px-3 py-2 border text-[11px] rounded-xl cursor-pointer" style={{ borderColor: overdueAlerts.length > 0 ? "#c4555544" : T.border, color: overdueAlerts.length > 0 ? "#c45555" : T.textSub }}>
            🚨 超過通知{overdueAlerts.length > 0 && <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] rounded-full text-[9px] font-bold text-white flex items-center justify-center" style={{ backgroundColor: "#c45555" }}>{overdueAlerts.length}</span>}
          </button>
          ...
          <button onClick={openThMemos} className="px-3 py-2 border text-[11px] rounded-xl cursor-pointer" style={{ borderColor: "#8b5cf644", color: "#8b5cf6" }}>💬 セラピストメモ</button>
          ...
          <button onClick={() => { const now = new Date(); const h = now.getHours(); const m = Math.ceil(now.getMinutes() / 10) * 10; const defaultStart = `${String(h).padStart(2,"0")}:${String(m % 60).padStart(2,"0")}`; const startMin = h * 60 + m; const defaultEnd = `${String(Math.floor((startMin+60)/60)%24).padStart(2,"0")}:${String((startMin+60)%60).padStart(2,"0")}`; setNewDate(selectedDate); setNewCourseId(0); setNewStart(defaultStart); setNewEnd(defaultEnd); setMsg(""); setNewTherapistId(0); setCustSearchQ(""); setShowCustSearch(true); supabase.from("customers").select("id,name,phone,rank").order("created_at",{ascending:false}).then(({data})=>{if(data)setCustList(data)}); }}
                      ...
                      return (
                        <div key={`res-${r.id}-${ri}`} className="res-block absolute top-[4px] bottom-[4px] rounded-lg cursor-pointer group transition-opacity"
                          ...
                          <div className="px-2 py-1 overflow-hidden h-full"
                            ...
                            <p className="font-medium truncate" style={{ fontSize: tcConfig.barFontSize, color: T.text }}>{r.customer_name}{(r as any).phone_confirm && <span style={{ marginLeft: 3, fontSize: 7, padding: "1px 4px", borderRadius: 4, backgroundColor: "#3d6b9f22", color: "#3d6b9f" }}>📞</span>}{cBadge && <span style={{ marginLeft: 4, fontSize: 7, padding: "1px 4px", borderRadius: 4, backgroundColor: color + "22", color }}>{cBadge}</span>}{tBadge && custSt !== therSt && <span style={{ marginLeft: 2, fontSize: 7, padding: "1px 4px", borderRadius: 4, backgroundColor: (dualStatusColors[therSt] || "#888780") + "22", color: dualStatusColors[therSt] || "#888780" }}>{tBadge}</span>}{(() => { const nom = (r as any).nomination || ""; if (!nom || nom === "指名なし") return null; const isHon = nom.includes("本"); const isP = nom.includes("P") || nom.includes("パネル"); const isFree = nom === "フリー" || nom.startsWith("フリー"); const nc = isHon ? "#c3a782" : isP ? "#85a8c4" : isFree ? "#378ADD" : "#9e9a91"; const nl = isHon ? "本指名" : isP ? "P指名" : isFree ? "フリー" : nom; return <span style={{ marginLeft: 2, fontSize: 7, padding: "1px 4px", borderRadius: 4, backgroundColor: nc + "22", color: nc }}>{nl}</span>; })()}</p>
                        ...
                        <div key={`fres-${r.id}`} className="res-block absolute rounded-lg cursor-grab transition-opacity"
                          ...
                          <div className="px-2 py-0.5 overflow-hidden h-full flex items-center gap-1">
                            ...
                            <div className="overflow-hidden">
                              <p className="font-medium truncate" style={{ fontSize: 10, color: T.text }}>{r.customer_name}{cBadge && <span style={{ marginLeft: 3, fontSize: 7, padding: "1px 4px", borderRadius: 4, backgroundColor: stColor + "22", color: stColor }}>{cBadge}</span>}{(() => { const nom = (r as any).nomination || ""; if (!nom || nom === "指名なし") return null; const isFree = nom === "フリー" || nom.startsWith("フリー"); const nc = isFree ? "#378ADD" : "#9e9a91"; const nl = isFree ? "フリー" : nom; return <span style={{ marginLeft: 2, fontSize: 7, padding: "1px 4px", borderRadius: 4, backgroundColor: nc + "22", color: nc }}>{nl}</span>; })()}</p>
      ...
      {showNewRes && (() => {
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowNewRes(false)}>
          <div className="rounded-2xl border p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto animate-[fadeIn_0.25s]" style={{ backgroundColor: T.card, borderColor: T.border }} onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[11px] mb-3" style={{ color: T.textFaint }}>{newCustName} 様の予約</p>
            {/* 📞 電話確認 */}
            <div className="mb-4">
            ...
            <div className="space-y-3">
              <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>日付</label><input type="date" value={newDate || selectedDate} onChange={(e) => setNewDate(e.target.value)} className="w-full px-3 py-2 rounded-xl text-[12px] outline-none cursor-pointer" style={inputStyle} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>開始時間</label><select value={newStart} onChange={(e) => handleStartChange(e.target.value)} className="w-full px-3 py-2 rounded-xl text-[12px] outline-none cursor-pointer" style={inputStyle}>{TIMES_10MIN.map((t) => (<option key={t} value={t}>{minutesToDisplay(timeToMinutes(t))}</option>))}</select></div>
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>終了時間{selectedCourse ? "（自動）" : ""}</label><select value={newEnd} onChange={(e) => setNewEnd(e.target.value)} disabled={!!selectedCourse} className="w-full px-3 py-2 rounded-xl text-[12px] outline-none cursor-pointer" style={{ ...inputStyle, color: selectedCourse ? "#c3a782" : T.text }}>{TIMES_10MIN.map((t) => (<option key={t} value={t}>{minutesToDisplay(timeToMinutes(t))}</option>))}</select></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>顧客名</label><input type="text" value={newCustName} onChange={(e) => setNewCustName(e.target.value)} className="w-full px-3 py-2 rounded-xl text-[12px] outline-none" style={inputStyle} /></div>
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>📞 電話番号</label><input type="tel" value={newCustPhone} onChange={(e) => setNewCustPhone(e.target.value)} placeholder="090-xxxx-xxxx" className="w-full px-3 py-2 rounded-xl text-[12px] outline-none" style={inputStyle} /></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>セラピスト <span style={{ color: "#c49885" }}>*</span></label><select value={newTherapistId} onChange={(e) => { const tid = Number(e.target.value); setNewTherapistId(tid); if (tid && newCustName) checkNgWarning(newCustName, tid); }} className="w-full px-3 py-2 rounded-xl text-[12px] outline-none cursor-pointer" style={{ ...inputStyle, borderColor: ngTherapistIdsForCust.has(newTherapistId) ? "#c45555" : "transparent", backgroundColor: ngTherapistIdsForCust.has(newTherapistId) ? "#c4555512" : T.cardAlt }}><option value={0}>選択</option>{therapists.filter((t) => !t.deleted_at).map((t) => (<option key={t.id} value={t.id} style={{ color: ngTherapistIdsForCust.has(t.id) ? "#c45555" : undefined }}>{t.name}{ngTherapistIdsForCust.has(t.id) ? " ⚠️NG" : ""}</option>))}</select>{ngTherapistIdsForCust.has(newTherapistId) && <p className="text-[10px] mt-1 font-bold" style={{ color: "#c45555" }}>⚠️ NG</p>}</div>
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>👤 受付スタッフ</label><select value={newStaffName} onChange={(e) => { setNewStaffName(e.target.value); localStorage.setItem("last_staff_name", e.target.value); }} className="w-full px-3 py-2 rounded-xl text-[12px] outline-none cursor-pointer" style={inputStyle}><option value="">未選択</option>{staffMembers.map((s) => (<option key={s.id} value={s.name}>{s.name}</option>))}</select></div>
              </div>
              <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>コース <span style={{ color: "#c49885" }}>* 必須</span></label><select value={newCourseId} onChange={(e) => handleCourseChange(Number(e.target.value))} className="w-full px-3 py-2 rounded-xl text-[11px] outline-none cursor-pointer" style={{ ...inputStyle, borderColor: !newCourseId ? "#c49885" : "transparent" }}><option value={0}>— コースを選択 —</option>{courses.map((c) => (<option key={c.id} value={c.id}>{c.name}（{c.duration}分 / {fmt(c.price)}）</option>))}</select></div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>指名</label><select value={newNomination} onChange={(e) => { const n = nominations.find(x=>x.name===e.target.value); setNewNomination(e.target.value); setNewNomFee(n?.price||0); }} className="w-full px-3 py-2 rounded-xl text-[11px] outline-none cursor-pointer" style={inputStyle}><option value="">指名なし</option>{nominations.map((n) => (<option key={n.id} value={n.name}>{n.name}（{fmt(n.price)}）</option>))}</select></div>
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>延長</label><select value={newExtension} onChange={(e) => { const ex = extensions.find(x=>x.name===e.target.value); setNewExtension(e.target.value); setNewExtPrice(ex?.price||0); setNewExtDur(ex?.duration||0); if (ex && selectedCourse && newStart) { setNewEnd(minutesToTime(timeToMinutes(newStart) + selectedCourse.duration + ex.duration)); } }} className="w-full px-3 py-2 rounded-xl text-[11px] outline-none cursor-pointer" style={inputStyle}><option value="">延長なし</option>{extensions.map((ex) => (<option key={ex.id} value={ex.name}>{ex.name}（{ex.duration}分 / {fmt(ex.price)}）</option>))}</select></div>
              </div>
              <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>割引（複数追加可）</label><div className="flex gap-2"><select id="newDiscSelect" className="flex-1 min-w-0 px-3 py-2 rounded-xl text-[11px] outline-none cursor-pointer" style={inputStyle}><option value="">— 割引を選択 —</option>{discounts.filter(d => !newDiscounts.some(nd => nd.name === d.name)).map((d) => (<option key={d.id} value={d.name}>{d.name}（{d.type==="percent" ? d.amount+"%" : fmt(d.amount)}）{(d as any).combinable ? " ✅併用可" : ""}{(d as any).newcomer_only ? " 🌟新人のみ" : ""}</option>))}</select><button onClick={() => { const sel = (document.getElementById("newDiscSelect") as HTMLSelectElement)?.value; if (!sel) return; const d = discounts.find(x=>x.name===sel); if (!d) return; const amt = d.type==="percent" ? Math.round(cp * d.amount / 100) : d.amount; setNewDiscounts([...newDiscounts, { name: sel, amount: amt }]); (document.getElementById("newDiscSelect") as HTMLSelectElement).value = ""; }} className="flex-shrink-0 px-4 py-2 rounded-xl text-[10px] cursor-pointer font-medium" style={{ backgroundColor: "#c4555518", color: "#c45555", border: "1px solid #c4555544" }}>+ 追加</button></div>{newDiscounts.some(nd => { const d = discounts.find(x => x.name === nd.name); return d && ((d as any).newcomer_only); }) && <div className="mt-2 px-3 py-2 rounded-xl text-[10px]" style={{ backgroundColor: "#f59e0b12", border: "1px solid #f59e0b33", color: "#f59e0b" }}>{newDiscounts.map(nd => { const d = discounts.find(x => x.name === nd.name); if (!d) return null; if ((d as any).newcomer_only) return <p key={nd.name}>⚠ {nd.name}: 新人限定</p>; return null; })}</div>}</div>
              ...
              <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>オプション（複数選択可）</label><div className="flex flex-wrap gap-1.5">{options.map((o) => { const sel = newOptions.some(x=>x.name===o.name); return <button key={o.id} onClick={() => { if (sel) setNewOptions(newOptions.filter(x=>x.name!==o.name)); else setNewOptions([...newOptions, { name: o.name, price: o.price }]); }} className="px-2.5 py-1 rounded-xl text-[9px] cursor-pointer" style={{ backgroundColor: sel ? "#85a8c418" : T.cardAlt, color: sel ? "#85a8c4" : T.textMuted, border: `1px solid ${sel ? "#85a8c4" : T.border}`, fontWeight: sel ? 600 : 400 }}>{o.name}（{fmt(o.price)}）</button>; })}</div>{newOptions.length > 0 && <p className="text-[9px] mt-1" style={{ color: "#85a8c4" }}>オプション合計: {fmt(newOptions.reduce((s,o)=>s+o.price,0))}</p>}</div>
              <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>備考</label><div style={{ position: "relative" }}><textarea value={newNotes} onChange={(e) => setNewNotes(e.target.value)} placeholder="メモ・備考を入力" rows={3} className="w-full px-3 py-2 rounded-xl text-[11px] outline-none resize-y" style={inputStyle} /><div style={{ position: "absolute", left: 12, right: 12, top: 36, borderTop: "1px dashed #f59e0b44", pointerEvents: "none" }} /><p className="text-[8px] mt-1" style={{ color: T.textFaint }}>⚠ 点線より上がタイムチャートに表示されます</p></div></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>💳 カード充当額（税抜）</label><input type="text" inputMode="numeric" value={newCardBase} onChange={(e) => setNewCardBase(e.target.value.replace(/[^0-9]/g, ""))} placeholder="0" className="w-full px-3 py-2 rounded-xl text-[12px] outline-none" style={inputStyle} />{parseInt(newCardBase) > 0 && <p className="text-[9px] mt-1" style={{ color: "#85a8c4" }}>カード決済額: {fmt(Math.round(parseInt(newCardBase) * 1.1))}（10%手数料込）</p>}</div>
                <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>📱 PayPay支払額</label><input type="text" inputMode="numeric" value={newPaypay} onChange={(e) => setNewPaypay(e.target.value.replace(/[^0-9]/g, ""))} placeholder="0" className="w-full px-3 py-2 rounded-xl text-[12px] outline-none" style={inputStyle} /></div>
              </div>
              <div className="rounded-xl p-4" style={{ backgroundColor: T.cardAlt, border: `1px solid ${T.border}` }}>
                ...
                <div className="space-y-1 text-[11px]">
                  ...
                  {newDiscounts.map((d,i) => <div key={`ndisc-${i}`} className="flex justify-between" style={{ color: "#c45555" }}><span>割引: {d.name}</span><span>-{fmt(d.amount)}</span></div>)}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setEditRes(null)}>
          <div className="rounded-2xl border p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto animate-[fadeIn_0.25s]" style={{ backgroundColor: T.card, borderColor: T.border }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div><label className="block text-[10px] mb-1" style={{ color: T.textSub }}>割引（複数追加可）</label><div className="flex gap-2"><select id="editDiscSelect" className="flex-1 min-w-0 px-3 py-2 rounded-xl text-[11px] outline-none cursor-pointer" style={inputStyle}><option value="">— 割引を選択 —</option>{discounts.filter(d => !editDiscounts.some(ed => ed.name === d.name)).map((d) => (<option key={d.id} value={d.name}>{d.name}（{d.type==="percent" ? d.amount+"%" : fmt(d.amount)}）{(d as any).combinable ? " ✅併用可" : ""}{(d as any).newcomer_only ? " 🌟新人のみ" : ""}</option>))}</select><button onClick={() => { const sel = (document.getElementById("editDiscSelect") as HTMLSelectElement)?.value; if (!sel) return; const d = discounts.find(x=>x.name===sel); if (!d) return; const amt = d.type==="percent" ? Math.round(eCp * d.amount / 100) : d.amount; setEditDiscounts([...editDiscounts, { name: sel, amount: amt }]); (document.getElementById("editDiscSelect") as HTMLSelectElement).value = ""; }} className="flex-shrink-0 px-4 py-2 rounded-xl text-[10px] cursor-pointer font-medium" style={{ backgroundColor: "#c4555518", color: "#c45555", border: "1px solid #c4555544" }}>+ 追加</button></div>{editDiscounts.some(ed => { const d = discounts.find(x => x.name === ed.name); return d && ((d as any).newcomer_only); }) && <div className="mt-2 px-3 py-2 rounded-xl text-[10px]" style={{ backgroundColor: "#f59e0b12", border: "1px solid #f59e0b33", color: "#f59e0b" }}>{editDiscounts.map(ed => { const d = discounts.find(x => x.name === ed.name); if (!d) return null; if ((d as any).newcomer_only) return <p key={ed.name}>⚠ {ed.name}: 新人限定</p>; return null; })}</div>}</div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={async () => {
                  ...
                  setNotifyInfo({ resId: editRes.id, custName: editCustName.trim(), custPhone: custInfo?.phone || "", custEmail: custInfo?.login_email || "", hasLine, isMember, date: editRes.date, startTime: editStart, endTime: editEnd, course: courseWithExt, therapistName: thName, therapistId: editTherapistId, total: eTotal, nomination: editNomination || "指名なし", discountName: editDiscounts.map(d => d.name).join(",") || "なし", extensionName: editExtension, storeName: st?.name || "", buildingName: bl?.name || "" });
      ...
      {settleTh && (() => {
        ...
        const giftBack = settleGiftBack;  // 🎁 投げ銭バック (pending な gift_payouts の合計)
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-xl p-4" style={{ backgroundColor: T.cardAlt }}>
                ...
                <div className="space-y-1 text-[11px]">
                  ...
                  {giftBack > 0 && <div className="flex justify-between" style={{ color: "#c96b83" }}><span>💝 情報配信報酬</span><span>+{fmt(giftBack)}</span></div>}
              ...
              <div className="rounded-xl p-4" style={{ backgroundColor: cashBalance < 0 ? "#c4555512" : "#85a8c412", border: `1px solid ${cashBalance < 0 ? "#c4555533" : "#85a8c433"}` }}>
                ...
                <div className="flex gap-2 mt-3" style={{ display: (settleUseReserve || settleUseReplenish) ? "none" : "flex" }}>
                ...
                {!settleUseReserve && !settleUseReplenish && (settleSalesCollected || settleChangeCollected) && <div className="grid grid-cols-2 gap-2"><button onClick={() => { setSettleSafeDeposited(false); }} className="px-3 py-2.5 rounded-xl text-[10px] cursor-pointer" style={{ backgroundColor: !settleSafeDeposited ? "#22c55e18" : T.cardAlt, color: !settleSafeDeposited ? "#22c55e" : T.textMuted, border: `1px solid ${!settleSafeDeposited ? "#22c55e" : T.border}`, fontWeight: !settleSafeDeposited ? 700 : 400 }}>{!settleSafeDeposited ? "✅ スタッフが回収" : "👤 スタッフが回収"}</button><button onClick={() => { setSettleSafeDeposited(true); }} className="px-3 py-2.5 rounded-xl text-[10px] cursor-pointer" style={{ backgroundColor: settleSafeDeposited ? "#a855f718" : T.cardAlt, color: settleSafeDeposited ? "#a855f7" : T.textMuted, border: `1px solid ${settleSafeDeposited ? "#a855f7" : T.border}`, fontWeight: settleSafeDeposited ? 700 : 400 }}>{settleSafeDeposited ? "✅ 金庫に投函" : "🔐 金庫に投函"}</button></div>}
                {settleUseReserve && (
                  <div className="rounded-lg p-2.5 mt-3 text-center" style={{ backgroundColor: "#d4687e12", border: "1px solid #d4687e33" }}>
                    <p className="text-[10px] font-medium" style={{ color: "#d4687e" }}>✅ 予備金補充で完結</p>
                    <p className="text-[9px] mt-0.5" style={{ color: T.textFaint }}>セラピストには全額渡済み・ルームに現金は残りません</p>
                  </div>
                )}
                {settleUseReplenish && (
                  <div className="rounded-lg p-2.5 mt-3 text-center" style={{ backgroundColor: "#22c55e12", border: "1px solid #22c55e33" }}>
                    <p className="text-[10px] font-medium" style={{ color: "#22c55e" }}>✅ 釣銭補充で完結</p>
                    <p className="text-[9px] mt-0.5" style={{ color: T.textFaint }}>不足分を釣銭として補充・セラピストには全額渡済み</p>
                  </div>
                )}
              </div>
              )}
...
${giftBack > 0 ? `<tr><td style="color:#c96b83">💝 情報配信報酬</td><td class="right" style="color:#c96b83">+&yen;${giftBack.toLocaleString()}</td><td style="font-size:11px;color:#666">配信視聴ファンからの応援報酬</td></tr>` : ""}
...
<button onClick={() => setShowNewTherapist(false)} className="px-5 py-2.5 border text-[11px] rounded-xl cursor-pointer" style={{ borderColor: T.border, color: T.textSub }}>キャンセル</button>
      ...
      {notifyInfo && (() => {
        ...
        const applyTemplate = (tpl: string) => {
          ...
          const vars: Record<string, string> = {
            ...
            "{店舗名}": ni.storeName || "Ange Spa", "{金額}": ni.total.toLocaleString(),
            "{送信者}": notifySender, "{セラピスト名}": ni.therapistName, "{場所URL}": locUrl,
            "{ルーム名}": roomName, "{ビル名}": buildingName,
            "{お客様リンク}": customerMypageUrl(cleanName),
          };
        ...
        // ── 通知ログ記録ヘルパー（notification_logs に INSERT） ──
        const logNotification = async (params: {
          channel: string;
          recipientType: "customer" | "therapist" | "other";
          body: string;
          subject?: string;
        }) => {
          try {
            await supabase.from("notification_logs").insert({
              ...
              recipient_name: params.recipientType === "customer" ? cleanName : (ni.therapistName || ""),
              recipient_phone: params.recipientType === "customer" ? (ni.custPhone || "") : "",
              recipient_email: params.recipientType === "customer" ? (ni.custEmail || "") : "",
              therapist_id: params.recipientType === "therapist" ? (ni.therapistId || null) : null,
              message_type: notifyMode === "summary" ? "summary" : notifyMode === "detail" ? "detail" : "",
              subject: params.subject || "",
              body: params.body,
              body_preview: params.body.slice(0, 100),
              reservation_id: ni.resId || null,
              reservation_date: ni.date || null,
              ...
              <div className="space-y-2">
                {notifyTab === "customer" ? (<>
                  ...
                  {ni.hasLine && <button onClick={async () => {
                    ...
                    await logNotification({ channel: "line", recipientType: "customer", body: msg });
                  ...
                  {ni.isMember && ni.custEmail && <button onClick={async () => {
                    ...
                    await logNotification({ channel: "gmail", recipientType: "customer", body: msg, subject: "【アンジュスパ】ご予約確認" });
                  ...
                  {ni.custPhone && <div className="flex gap-2">
                    <button onClick={async () => {
                      const msg = await prepareCustomerNotify(custMsg);
                      navigator.clipboard.writeText(msg);
                      await logNotification({ channel: "sms1", recipientType: "customer", body: msg });
                    ...
                    <button onClick={async () => {
                      ...
                      await logNotification({ channel: "sms2", recipientType: "customer", body: msg });
                  ...
                  <button onClick={async () => {
                    const msg = await prepareCustomerNotify(custMsg);
                    navigator.clipboard.writeText(msg);
                    await logNotification({ channel: "copy", recipientType: "customer", body: msg });
                  ...
                  <button onClick={async () => {
                    navigator.clipboard.writeText(staffMsg);
                    await logNotification({ channel: "therapist_line", recipientType: "therapist", body: staffMsg });
                    toast.show("セラピスト向けメッセージをコピーしました！", "success");
            ...
            <div className="px-6 py-4 space-y-4">
                ...
                <div key={td.therapistId} className="rounded-xl border" style={{ borderColor: bulkCopied[td.therapistId] ? "#4a7c5944" : T.border }}>
                  ...
                  <div className="flex items-center justify-between px-4 py-3" style={{ backgroundColor: bulkCopied[td.therapistId] ? "#4a7c5908" : T.cardAlt, borderRadius: "12px 12px 0 0" }}>
                    <div className="flex items-center gap-3">
                      ...
                      {bulkCopied[td.therapistId] && <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ backgroundColor: "#4a7c5918", color: "#4a7c59" }}>✅ 送信済</span>}
                    </div>
                    <div className="flex items-center gap-1.5">
                      <button onClick={async () => {
                        ...
                        try {
                          await supabase.from("notification_logs").insert({
                            channel: "bulk_therapist",
                            recipient_type: "therapist",
                            recipient_name: td.therapistName,
                            therapist_id: td.therapistId,
                            message_type: "bulk",
                            body: td.message,
                            body_preview: td.message.slice(0, 100),
                      ...
                      }} className="px-3 py-2 rounded-xl text-[11px] cursor-pointer" style={{
                        backgroundColor: T.cardAlt, color: T.textSub, border: `1px solid ${T.border}`
                      }}>
                        📋 コピー
                      </button>
                      <button onClick={() => setBulkActiveSend(td)} className="px-4 py-2 rounded-xl text-[11px] cursor-pointer font-medium" style={{
                        backgroundColor: bulkCopied[td.therapistId] ? "#4a7c5918" : "#06C75518",
                        color: bulkCopied[td.therapistId] ? "#4a7c59" : "#06C755",
                        border: `1px solid ${bulkCopied[td.therapistId] ? "#4a7c5944" : "#06C75544"}`
                      }}>
                        {bulkCopied[td.therapistId] ? "✅ 済" : "💬 LINE送信"}
                      </button>
                    </div>
                  </div>
              ...
              {bulkData.length > 0 && (
                <div className="pt-2 space-y-2">
                  <button onClick={() => {
                    const allDone = bulkData.every(d => bulkCopied[d.therapistId]);
                    if (allDone) { toast.show("全員分送信済みです", "info"); return; }
                    const remaining = bulkData.filter(d => !bulkCopied[d.therapistId]);
                    if (remaining.length > 0) {
                      setBulkActiveSend(remaining[0]);
                    }
                  }} className="w-full py-3 rounded-xl text-[13px] font-medium cursor-pointer text-white" style={{
                    backgroundColor: "#06C755"
                  }}>
                    💬 次の未送信をLINE送信（{bulkData.filter(d => !bulkCopied[d.therapistId]).length}名残り）
                  </button>
      ...
      {/* ===== 一括通知 LINE送信ポップアップ（data-tm方式） ===== */}
      {bulkActiveSend && (
        <div
          className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[60] p-4"
          onClick={() => { setBulkCopied(prev => ({ ...prev, [bulkActiveSend.therapistId]: true })); setBulkActiveSend(null); }}
          data-tm-notify="true"
          data-tm-therapist={bulkActiveSend.therapistName}
          data-tm-custname=""
          data-tm-phone=""
        >
          <div className="rounded-2xl border w-full max-w-lg max-h-[80vh] overflow-y-auto animate-[fadeIn_0.25s]" style={{ backgroundColor: T.card, borderColor: T.border }} onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${T.border}` }}>
              <div>
                <h2 className="text-[15px] font-medium">📩 セラピスト通知 LINE送信</h2>
                <p className="text-[11px] mt-0.5" style={{ color: T.textMuted }}>{bulkActiveSend.therapistName} | {bulkActiveSend.reservations.length}件の予約</p>
              </div>
              <button onClick={() => { setBulkCopied(prev => ({ ...prev, [bulkActiveSend.therapistId]: true })); setBulkActiveSend(null); }} className="text-[14px] cursor-pointer p-2" style={{ color: T.textSub, background: "none", border: "none" }}>✕</button>
            </div>
            <div className="px-6 py-4">
              <div data-tm-preview="true" className="rounded-xl p-4 mb-4 text-[11px] whitespace-pre-wrap leading-relaxed max-h-[300px] overflow-y-auto" style={{ backgroundColor: T.cardAlt, color: T.textSub, fontFamily: "var(--font-mono, monospace)" }}>
                {bulkActiveSend.message}
              </div>
              <div className="space-y-2">
                <button onClick={async () => {
                  navigator.clipboard.writeText(bulkActiveSend.message);
                  try {
                    await supabase.from("notification_logs").insert({
                      channel: "bulk_therapist",
                      recipient_type: "therapist",
                      recipient_name: bulkActiveSend.therapistName,
                      therapist_id: bulkActiveSend.therapistId,
                      message_type: "bulk",
                      body: bulkActiveSend.message,
                      body_preview: bulkActiveSend.message.slice(0, 100),
                      reservation_date: selectedDate,
                      sent_by_staff_id: activeStaff?.id || null,
                      sent_by_name: activeStaff?.name || "",
                      status: "copied",
                  ...
                  toast.show(`${bulkActiveSend.therapistName}さんの通知をコピーしました！`, "success");
                }} className="w-full py-3 rounded-xl text-[13px] font-medium cursor-pointer" style={{ backgroundColor: "#85a8c418", color: "#85a8c4", border: "1px solid #85a8c444" }}>💬 セラピストLINE用コピー</button>
                <button onClick={() => { setBulkCopied(prev => ({ ...prev, [bulkActiveSend.therapistId]: true })); setBulkActiveSend(null); }} className="w-full py-2.5 rounded-xl text-[12px] cursor-pointer" style={{ color: T.textMuted }}>閉じる（送信済みにする）</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* セラピストメモ一覧（お客様が書いたメモ） */}
      {showThMemos && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowThMemos(false)}>
          <div className="rounded-2xl shadow-2xl w-full max-w-[560px] max-h-[85vh] overflow-hidden flex flex-col" style={{ backgroundColor: T.card }} onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${T.border}`, background: "linear-gradient(135deg, #8b5cf618, #7c3aed18)" }}>
              <div>
                <h2 className="text-[16px] font-semibold" style={{ color: T.text }}>💬 セラピストメモ一覧</h2>
                <p className="text-[10px] mt-0.5" style={{ color: T.textFaint }}>お客様が記録したセラピストへのメモ（新しい順）</p>
              </div>
              <button onClick={() => setShowThMemos(false)} className="text-[16px] cursor-pointer p-1.5" style={{ color: T.textSub }}>✕</button>
            </div>
            <div className="px-6 py-3" style={{ borderBottom: `1px solid ${T.border}` }}>
              <input type="text" value={thMemoSearch} onChange={e => setThMemoSearch(e.target.value)} placeholder="お客様名・セラピスト名・メモ内容で検索" className="w-full px-3 py-2 rounded-xl text-[12px] outline-none" style={{ backgroundColor: T.cardAlt, color: T.text, border: `1px solid ${T.border}` }} />
            </div>
            <div className="overflow-y-auto flex-1">
              {(() => {
                const filtered = thMemos.filter(m => {
                  if (!thMemoSearch) return true;
                  const q = thMemoSearch.toLowerCase();
                  const thName = therapists.find(t => t.id === m.therapist_id)?.name || "";
                  return (m.customer_name || "").toLowerCase().includes(q) || thName.toLowerCase().includes(q) || (m.memo || "").toLowerCase().includes(q);
                });
                if (filtered.length === 0) return <p className="text-center text-[12px] py-10" style={{ color: T.textFaint }}>メモがありません</p>;
                return filtered.map(m => {
                  const th = therapists.find(t => t.id === m.therapist_id);
                  const dateStr = m.res_date ? (() => { const dt = new Date(m.res_date + "T00:00:00"); const days = ["日","月","火","水","木","金","土"]; return `${dt.getMonth()+1}/${dt.getDate()}(${days[dt.getDay()]})`; })() : "";
                  return (
                    <div key={m.id} className="px-6 py-3 cursor-pointer hover:opacity-80" style={{ borderBottom: `1px solid ${T.border}` }}
                      onClick={() => { setShowThMemos(false); router.push("/dashboard?page=" + encodeURIComponent("顧客") + "&search=" + encodeURIComponent(m.customer_name || "")); }}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[13px] font-medium" style={{ color: "#8b5cf6" }}>{m.customer_name || "不明"}</span>
                          <span className="text-[10px]" style={{ color: T.textMuted }}>→</span>
                          <span className="text-[12px] font-medium" style={{ color: T.accent }}>{th?.name || (m.therapist_id === 0 ? "フリー" : "不明")}</span>
                          {m.rating > 0 && <span className="text-[9px]" style={{ color: "#f59e0b" }}>{"★".repeat(m.rating)}{"☆".repeat(5 - m.rating)}</span>}
                        </div>
                        <span className="text-[9px]" style={{ color: T.textFaint }}>{dateStr}</span>
                      </div>
                      {m.memo && <p className="text-[11px] leading-relaxed" style={{ color: T.textSub }}>{m.memo.length > 120 ? m.memo.substring(0, 120) + "..." : m.memo}</p>}
                      {m.res_course && <p className="text-[9px] mt-1" style={{ color: T.textMuted }}>{m.res_course}</p>}
                      <p className="text-[8px] mt-1 text-right" style={{ color: "#8b5cf6" }}>→ お客様詳細を開く</p>
                    </div>
                  );
                });
              })()}
            ...
            <div className="px-6 py-3 text-center" style={{ borderTop: `1px solid ${T.border}` }}>
              <span className="text-[10px]" style={{ color: T.textFaint }}>{thMemos.length}件のメモ</span>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="rounded-2xl border shadow-2xl p-4" style={{ backgroundColor: dark ? "#2a1a1a" : "#fff5f5", borderColor: "#c4555566", boxShadow: "0 0 30px rgba(196,85,85,0.3)" }}>
            ...
            <div className="space-y-2">
              {overdueAlerts.map(a => {
                ...
                const sent = overdueSentRef.current.has(key);
                return (
                <div key={key} className="px-3 py-2.5 rounded-lg text-[11px]" style={{ backgroundColor: "#c4555510", border: "1px solid #c4555520" }}>
                  <div className="flex items-center justify-between">
                    <div>
                      <span style={{ color: a.type === "phone" ? "#3d6b9f" : "#c45555", fontWeight: 600 }}>{{ start: "🔔 来店未確認", end: "🚪 退室未確認", summary: "📩 概要送信済", detail: "📋 詳細送信済", phone: "📞 電話確認" }[a.type]}</span>
                      <span style={{ color: T.text, marginLeft: 6 }}>{a.customerName}</span>
                      <span style={{ color: T.textMuted, marginLeft: 4 }}>({a.therapistName} / {a.time?.slice(0,5)}〜)</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => {
                      const msgMap: Record<string,string> = {
                        start: `🔔 入室確認のお願い 🔔\n\n${a.customerName}様の開始時間（${a.time?.slice(0,5)}〜）を${tcConfig.alertStartMin}分過ぎております。\n\n⚠️ 入室ボタンの押し忘れはございませんか？\n\n👇 こちらからご確認ください\n${window.location.origin}/mypage`,
                        end: `🚪 退室確認のお願い 🚪\n\n${a.customerName}様の終了時間（${a.time?.slice(0,5)}）を${tcConfig.alertEndMin}分過ぎております。\n\n⚠️ 退室ボタンの押し忘れはございませんか？\n\n👇 こちらからご確認ください\n${window.location.origin}/mypage`,
                        phone: `📞 電話確認のお願い 📞\n\n${a.customerName}様のご予約（${a.time?.slice(0,5)}〜）の${tcConfig.alertPhoneMin}分前です。\n\n⚠️ お客様からのお電話はございましたか？\n\nお電話がない場合はお客様にご確認をお願いします。`,
                      };
                      const msg = msgMap[a.type] || `${a.customerName}様のご予約（${a.time?.slice(0,5)}〜）に関する通知です。`;
                      window.dispatchEvent(new CustomEvent("TMANAGE_LINE_OVERDUE_SEND", { detail: { name: a.therapistName, message: msg } }));
                      overdueSentRef.current.add(key);
                      toast.show(`${a.therapistName}にLINE準備しました`, "success");
                    }} className="text-[9px] px-3 py-1 rounded cursor-pointer" style={{ backgroundColor: "#22c55e18", color: "#22c55e", border: "1px solid #22c55e30" }}>{sent ? "↻ 再送信" : "📩 LINE送信"}</button>
                    {sent && <span className="text-[8px] px-1.5 py-0.5 rounded" style={{ backgroundColor: "#22c55e18", color: "#22c55e" }}>✅ 送信済</span>}
                    <button onClick={() => {
                      overdueDismissedRef.current.add(key);
                      setOverdueAlerts(prev => prev.filter(x => `${x.id}-${x.type}` !== key));
                    }} className="text-[9px] px-2 py-1 rounded cursor-pointer ml-auto" style={{ color: T.textMuted, backgroundColor: T.cardAlt, border: `1px solid ${T.border}` }}>✕ 取消</button>
                  </div>
                </div>
                );
              })}
            </div>
            <p className="text-[9px] mt-2" style={{ color: T.textMuted }}>「LINE送信」でメッセージが準備されます。LINEタブで送信ボタンを押してください。</p>
          </div>
        </div>
      )}
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowTcSettings(false)}>
            <div className="rounded-2xl border w-full max-w-[520px] max-h-[85vh] overflow-y-auto mx-4 animate-[fadeIn_0.2s]" style={{ backgroundColor: T.card, borderColor: T.border }} onClick={e => e.stopPropagation()}>
              ...
              <div className="px-6 py-4 space-y-6">
                ...
                {/* 超過通知設定 */}
                <div>
                  <h4 className="text-[13px] font-medium mb-3">🚨 超過通知設定</h4>
                  <div className="space-y-3">
                    {[
                      { label: "🔔 来店遅れ", desc: "予約開始時間後", enabledKey: "alertStartEnabled" as const, minKey: "alertStartMin" as const, suffix: "分後に通知" },
                      { label: "🚪 終了遅れ", desc: "終了時間後", enabledKey: "alertEndEnabled" as const, minKey: "alertEndMin" as const, suffix: "分後に通知" },
                      { label: "📩 概要送信済既読", desc: "予約時間の", enabledKey: "alertSummaryEnabled" as const, minKey: "alertSummaryMin" as const, suffix: "分前に通知" },
                      { label: "📋 詳細送信済既読", desc: "予約時間の", enabledKey: "alertDetailEnabled" as const, minKey: "alertDetailMin" as const, suffix: "分前に通知" },
                      { label: "📞 電話確認", desc: "予約時間の", enabledKey: "alertPhoneEnabled" as const, minKey: "alertPhoneMin" as const, suffix: "分前に通知" },
                    ].map(item => (
                      <div key={item.enabledKey} className="flex items-center gap-3 py-2 px-3 rounded-lg" style={{ backgroundColor: tcConfig[item.enabledKey] ? T.accent + "08" : T.cardAlt, border: `1px solid ${tcConfig[item.enabledKey] ? T.accent + "30" : T.border}` }}>
                        <input type="checkbox" checked={tcConfig[item.enabledKey] as boolean} onChange={e => updateCfg(item.enabledKey, e.target.checked)} className="w-4 h-4 accent-[#c3a782] flex-shrink-0" />
                        <div className="flex-1 min-w-0">
                          <span className="text-[12px] font-medium">{item.label}</span>
                          <div className="flex items-center gap-1 mt-1">
                            <span className="text-[10px]" style={{ color: T.textMuted }}>{item.desc}</span>
                            <input type="number" min={1} max={120} value={tcConfig[item.minKey] as number} onChange={e => updateCfg(item.minKey, Number(e.target.value) || 5)} disabled={!tcConfig[item.enabledKey]} className="w-[50px] px-2 py-1 rounded text-[11px] text-center outline-none" style={{ backgroundColor: T.cardAlt, border: `1px solid ${T.border}`, color: tcConfig[item.enabledKey] ? T.text : T.textFaint }} />
                            <span className="text-[10px]" style={{ color: T.textMuted }}>{item.suffix}</span>
                          </div>
                        </div>
                        <span className="text-[9px] px-2 py-0.5 rounded-full flex-shrink-0" style={{ backgroundColor: tcConfig[item.enabledKey] ? "#22c55e18" : "#c4555518", color: tcConfig[item.enabledKey] ? "#22c55e" : "#c45555" }}>{tcConfig[item.enabledKey] ? "ON" : "OFF"}</span>
                      </div>
                    ))}
                  </div>
                  <p className="text-[9px] mt-2" style={{ color: T.textFaint }}>※ 電話確認はオーダー編集画面で「📞 電話確認」をONにした予約のみ対象です</p>
                </div>
