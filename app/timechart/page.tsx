...
<button onClick={() => setShowNewTherapist(false)} className="px-5 py-2.5 border text-[11px] rounded-xl cursor-pointer" style={{ borderColor: T.border, color: T.textSub }}>キャンセル</button>
      ...
      {notifyInfo && (() => {
        ...
        // ── 確認リンク生成＆ステータス更新 ──
        const prepareCustomerNotify = async (msg: string, isUrlIncluded: boolean) => {
          if (!ni.resId) return msg;  // 新規予約（IDなし）はリンクなし
          try {
            // トークン取得 or 生成
            ...
            // ステータスを未読に更新（まだ送信前の場合）
            const cs = resData?.customer_status || "unsent";
            const newCs = isUrlIncluded
              ? (cs === "unsent" || cs === "web_reservation" ? "summary_unread" : cs)
              : (cs === "unsent" || cs === "web_reservation" || cs === "summary_unread" || cs === "summary_read" ? "detail_unread" : cs);
            if (newCs !== cs) {
              ...
              <div className="space-y-2">
                {notifyTab === "customer" ? (<>
                  ...
                  {ni.hasLine && <button onClick={async () => {
                    const msg = await prepareCustomerNotify(custMsg, showUrl);
                  ...
                  {/* SMS（電話番号がある場合 — 常に表示） */}
                  {ni.custPhone && <button onClick={async () => {
                    const msg = await prepareCustomerNotify(custMsg, showUrl);
                    navigator.clipboard.writeText(msg);
                    toast.show(`SMS用コピー完了（${ni.custPhone}）`, "success");
                  }} className="w-full py-3 rounded-xl text-[13px] font-medium cursor-pointer" style={{ backgroundColor: "#f59e0b18", color: "#f59e0b", border: "1px solid #f59e0b44" }}>📱 SMS用コピー（{ni.custPhone}）</button>}
