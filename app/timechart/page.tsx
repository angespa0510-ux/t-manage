...
export default function TimeChart() {
  ...
  const [settleUseReserve, setSettleUseReserve] = useState(false);  // 🏛 豊橋予備金から補充するか
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-xl p-4" style={{ backgroundColor: cashBalance < 0 ? "#c4555512" : "#85a8c412", border: `1px solid ${cashBalance < 0 ? "#c4555533" : "#85a8c433"}` }}>
                ...
                <div className="space-y-1 text-[11px]">
                  ...
                  {cashBalance < 0 && <div className="mt-2 space-y-2">
                    <p className="text-[10px] font-medium px-2 py-1.5 rounded" style={{ backgroundColor: "#c4555518", color: "#c45555" }}>⚠ 残高がマイナスです。予備金から {fmt(Math.abs(cashBalance))} を補充してください</p>
                    <button onClick={() => setSettleUseReserve(!settleUseReserve)} className="w-full px-3 py-2.5 rounded-xl text-[11px] cursor-pointer" style={{ backgroundColor: settleUseReserve ? "#d4687e18" : T.cardAlt, color: settleUseReserve ? "#d4687e" : T.textSub, border: `1px solid ${settleUseReserve ? "#d4687e" : T.border}`, fontWeight: settleUseReserve ? 600 : 400 }}>
                ...
                <div className="flex gap-2 mt-3" style={{ display: settleUseReserve ? "none" : "flex" }}>
                ...
                {!settleUseReserve && (settleSalesCollected || settleChangeCollected) && <div className="grid grid-cols-2 gap-2"><button onClick={() => { setSettleSafeDeposited(false); }} className="px-3 py-2.5 rounded-xl text-[10px] cursor-pointer" style={{ backgroundColor: !settleSafeDeposited ? "#22c55e18" : T.cardAlt, color: !settleSafeDeposited ? "#22c55e" : T.textMuted, border: `1px solid ${!settleSafeDeposited ? "#22c55e" : T.border}`, fontWeight: !settleSafeDeposited ? 700 : 400 }}>{!settleSafeDeposited ? "✅ スタッフが回収" : "👤 スタッフが回収"}</button><button onClick={() => { setSettleSafeDeposited(true); }} className="px-3 py-2.5 rounded-xl text-[10px] cursor-pointer" style={{ backgroundColor: settleSafeDeposited ? "#a855f718" : T.cardAlt, color: settleSafeDeposited ? "#a855f7" : T.textMuted, border: `1px solid ${settleSafeDeposited ? "#a855f7" : T.border}`, fontWeight: settleSafeDeposited ? 700 : 400 }}>{settleSafeDeposited ? "✅ 金庫に投函" : "🔐 金庫に投函"}</button></div>}
                {settleUseReserve && (
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={async () => { setSettleSaving(true); const reserveUsedAmt = (settleUseReserve && cashBalance < 0) ? Math.abs(cashBalance) : 0; /* 予備金使用時は売上回収・釣銭回収は自動で完結扱い、金庫投函はなし */ const effectiveSalesCollected = settleUseReserve ? true : settleSalesCollected; const effectiveChangeCollected = settleUseReserve ? true : settleChangeCollected; const effectiveSafeDeposited = settleUseReserve ? false : settleSafeDeposited; await supabase.from("therapist_daily_settlements").upsert({ therapist_id: settleTh.id, date: selectedDate, total_sales: totalSales, total_back: totalBack + salaryBonus + totalNom + totalOptBack + totalExtBack, total_nomination: totalNom, total_options: totalOpt, total_extension: totalExt, total_discount: totalDisc, total_card: totalCard, total_paypay: totalPaypay, total_cash: totalCash, order_count: tRes.length, is_settled: true, adjustment: adj, adjustment_note: settleAdjNote.trim(), invoice_deduction: invoiceDed, has_invoice: settleInvoice, withholding_tax: withholding, final_payment: finalPay, welfare_fee: welfareFee, transport_fee: transportFee, room_id: ra?.room_id || 0, sales_collected: effectiveSalesCollected, change_collected: effectiveChangeCollected, safe_deposited: effectiveSafeDeposited, reserve_used_amount: reserveUsedAmt }, { onConflict: "therapist_id,date" }); if (ra && effectiveSalesCollected) { for (const p of pastUncollected) { if (!p.sales_collected) { await supabase.from("therapist_daily_settlements").update({ sales_collected: true }).eq("room_id", ra.room_id).eq("date", p.date).eq("sales_collected", false); } } } if (ra && effectiveChangeCollected) { for (const p of pastUncollected) { if (!p.change_collected) { await supabase.from("therapist_daily_settlements").update({ change_collected: true }).eq("room_id", ra.room_id).eq("date", p.date).eq("change_collected", false); } } } /* 豊橋予備金の自動連動: 既存の精算連動エントリを削除 → 必要なら新規作成 */ const linkNote = `精算連動:${settleTh.name}`; await supabase.from("toyohashi_reserve_movements").delete().eq("therapist_id", settleTh.id).eq("movement_date", selectedDate).eq("movement_type", "withdraw").eq("note", linkNote); if (reserveUsedAmt > 0) { await supabase.from("toyohashi_reserve_movements").insert({ movement_date: selectedDate, movement_type: "withdraw", amount: reserveUsedAmt, therapist_id: settleTh.id, recorded_by_name: "精算モーダル自動連動", note: linkNote }); } toast.show(reserveUsedAmt > 0 ? `清算を確定しました（豊橋予備金から ${fmt(reserveUsedAmt)} 立替記録）` : "清算を確定しました", "success"); setSettleSaving(false); setSettleTh(null); fetchData(); }} disabled={settleSaving} className="px-5 py-2.5 bg-gradient-to-r from-[#c3a782] to-[#b09672] text-white text-[11px] rounded-xl cursor-pointer disabled:opacity-60">{settleSaving ? "保存中..." : "清算確定"}</button>
