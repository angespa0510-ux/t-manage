        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowNewRes(false)}>
          <div className="rounded-2xl border p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto animate-[fadeIn_0.25s]" style={{ backgroundColor: T.card, borderColor: T.border }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[11px] mb-1.5" style={{ color: T.textSub }}>セラピスト <span style={{ color: "#c49885" }}>*</span></label><select value={newTherapistId} onChange={(e) => { const tid = Number(e.target.value); setNewTherapistId(tid); if (tid && newCustName) checkNgWarning(newCustName, tid); }} className="w-full px-3 py-2.5 rounded-xl text-[12px] outline-none cursor-pointer" style={{ ...inputStyle, borderColor: ngTherapistIdsForCust.has(newTherapistId) ? "#c45555" : "transparent", backgroundColor: ngTherapistIdsForCust.has(newTherapistId) ? "#c4555512" : T.cardAlt }}><option value={0}>選択</option>{therapists.map((t) => (<option key={t.id} value={t.id} style={{ color: ngTherapistIdsForCust.has(t.id) ? "#c45555" : undefined }}>{t.name}{ngTherapistIdsForCust.has(t.id) ? " ⚠️NG" : ""}</option>))}</select>{ngTherapistIdsForCust.has(newTherapistId) && <p className="text-[10px] mt-1 font-bold" style={{ color: "#c45555" }}>⚠️ このセラピストはこのお客様をNGにしています！</p>}</div>
