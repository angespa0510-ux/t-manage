...
export default function TimeChart() {
  ...
  const overdueSentRef = useRef<Set<string>>(new Set()); // LINE送信済みトラッキング
  const overdueDismissedRef = useRef<Set<string>>(new Set()); // 個別取り消し済み
  useEffect(() => {
    const check = () => {
      const now = new Date();
      ...
      for (const r of reservations) {
        ...
        const startKey = `${r.id}-start`;
        const endKey = `${r.id}-end`;
        if (nowMin >= startMin + 5 && cs !== "serving" && cs !== "completed" && !overdueDismissedRef.current.has(startKey)) {
          alerts.push({ id: r.id, type: "start", customerName: r.customer_name, therapistName: tName, time: r.start_time });
        }
        if (nowMin >= endMin + 5 && cs !== "completed" && cs === "serving" && !overdueDismissedRef.current.has(endKey)) {
              ...
              {overdueAlerts.map(a => {
                ...
                <div key={key} className="px-3 py-2.5 rounded-lg text-[11px]" style={{ backgroundColor: "#c4555510", border: "1px solid #c4555520" }}>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => {
                      const msg = a.type === "start"
                        ? `【入室確認】${a.customerName}様の開始時間（${a.time?.slice(0,5)}〜）を5分過ぎています。入室ボタンの押し忘れはございませんか？マイページからご確認をお願いします。`
                        : `【退室確認】${a.customerName}様の終了時間（${a.time?.slice(0,5)}）を5分過ぎています。退室ボタンの押し忘れはございませんか？マイページからご確認をお願いします。`;
