        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowNewRes(false)}>
          <div className="rounded-2xl border p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto animate-[fadeIn_0.25s]" style={{ backgroundColor: T.card, borderColor: T.border }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div><label className="block text-[11px] mb-1.5" style={{ color: T.textSub }}>割引（複数追加可）</label><div className="flex gap-2"><select id="newDiscSelect" className="flex-1 min-w-0 px-3 py-2.5 rounded-xl text-[12px] outline-none cursor-pointer" style={inputStyle}><option value="">— 割引を選択 —</option>{discounts.filter(d => !newDiscounts.some(nd => nd.name === d.name)).map((d) => (<option key={d.id} value={d.name} style={{ color: !(d as any).web_available ? "#c45555" : undefined }}>{d.name}（{d.type==="percent" ? d.amount+"%" : fmt(d.amount)}）{!(d as any).web_available ? " ⚠ネット不可" : ""}{!(d as any).combinable ? " ⚠併用不可" : ""}{(d as any).newcomer_only ? " 🌟新人のみ" : ""}</option>))}</select><button onClick={() => { const sel = (document.getElementById("newDiscSelect") as HTMLSelectElement)?.value; if (!sel) return; const d = discounts.find(x=>x.name===sel); if (!d) return; const amt = d.type==="percent" ? Math.round(cp * d.amount / 100) : d.amount; setNewDiscounts([...newDiscounts, { name: sel, amount: amt }]); (document.getElementById("newDiscSelect") as HTMLSelectElement).value = ""; }} className="flex-shrink-0 px-4 py-2.5 rounded-xl text-[11px] cursor-pointer font-medium" style={{ backgroundColor: "#c4555518", color: "#c45555", border: "1px solid #c4555544" }}>+ 追加</button></div>{newDiscounts.some(nd => { const d = discounts.find(x => x.name === nd.name); return d && (!(d as any).web_available || !(d as any).combinable || (d as any).newcomer_only); }) && <div className="mt-2 px-3 py-2 rounded-xl text-[10px]" style={{ backgroundColor: "#f59e0b12", border: "1px solid #f59e0b33", color: "#f59e0b" }}>{newDiscounts.map(nd => { const d = discounts.find(x => x.name === nd.name); if (!d) return null; const warns: string[] = []; if (!(d as any).web_available) warns.push("ネット予約不可"); if (!(d as any).combinable) warns.push("併用不可"); if ((d as any).newcomer_only) warns.push("新人限定"); return warns.length > 0 ? <p key={nd.name}>⚠ {nd.name}: {warns.join("・")}</p> : null; })}</div>}</div>
