  ...
  useEffect(() => {
    const check = () => {
      ...
      if (alerts.length > 0 && alerts.length !== overdueAlerts.length) {
        ...
        // LINE自動送信（Chrome拡張経由・未送信のみ）
        for (const a of alerts) {
          const key = `${a.id}-${a.type}`;
          if (overdueSentRef.current.has(key)) continue;
          overdueSentRef.current.add(key);
          const msg = a.type === "start"
            ? `【入室確認】${a.customerName}様の開始時間（${a.time?.slice(0,5)}〜）を${5}分過ぎています。入室ボタンの押し忘れはございませんか？マイページからご確認をお願いします。`
            : `【退室確認】${a.customerName}様の終了時間（${a.time?.slice(0,5)}）を${5}分過ぎています。退室ボタンの押し忘れはございませんか？マイページからご確認をお願いします。`;
          try {
          ...
          <div className="rounded-2xl border shadow-2xl p-4" style={{ backgroundColor: dark ? "#2a1a1a" : "#fff5f5", borderColor: "#c4555566", boxShadow: "0 0 30px rgba(196,85,85,0.3)" }}>
            ...
            <div className="space-y-2">
              {overdueAlerts.map(a => {
                ...
                return (
                <div key={key} className="px-3 py-2 rounded-lg text-[11px]" style={{ backgroundColor: "#c4555510", border: "1px solid #c4555520" }}>
                  <div className="flex items-center gap-2">
                    <span style={{ color: "#c45555", fontWeight: 600 }}>{a.type === "start" ? "🔔 来店未確認" : "🚪 退室未確認"}</span>
                    <span style={{ color: T.text }}>{a.customerName}</span>
                    <span style={{ color: T.textMuted }}>({a.therapistName} / {a.time?.slice(0,5)}〜)</span>
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    {sent && <span className="text-[8px] px-1.5 py-0.5 rounded" style={{ backgroundColor: "#22c55e18", color: "#22c55e" }}>✅ LINE送信済</span>}
                    <button onClick={() => {
                      const msg = a.type === "start"
                      ...
                      window.dispatchEvent(new CustomEvent("TMANAGE_LINE_OVERDUE_SEND", { detail: { name: a.therapistName, message: msg } }));
                      overdueSentRef.current.add(key);
                      toast.show(`${a.therapistName}にLINE送信しました`, "success");
                    }} className="text-[8px] px-2 py-0.5 rounded cursor-pointer" style={{ backgroundColor: "#22c55e18", color: "#22c55e", border: "1px solid #22c55e30" }}>{sent ? "↻ 再送信" : "📩 LINE送信"}</button>
                  </div>
                </div>
                );
              })}
            </div>
            <p className="text-[9px] mt-2" style={{ color: T.textMuted }}>5分以上ステータスが更新されていません。LINE自動送信済みの場合は ✅ が表示されます。</p>
          </div>
        </div>
      )}
