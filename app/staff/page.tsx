...
function StaffPageInner() {
  ...
  const [addRealName, setAddRealName] = useState(""); const [addEntryDate, setAddEntryDate] = useState("");
  ...
  const [editRealName, setEditRealName] = useState(""); const [editEntryDate, setEditEntryDate] = useState("");
                        ...
                        <tr key={s.id} style={{ backgroundColor: idx % 2 === 0 ? "transparent" : (dark ? T.cardAlt : "#f8f6f3") }}>
                          ...
                          {isPresident && (
                            <td className="px-3 py-2 text-center" style={{ borderBottom: `1px solid ${T.border}` }}>
                              <button
                                onClick={() => {
                                  setEditStaff(s); setEditName(s.name); setEditRealName(s.real_name||""); setEditEntryDate(s.entry_date||""); setEditPhone(s.phone||""); setEditEmail(s.email||""); setEditRole(s.role); setEditPosition(s.company_position||"業務委託"); setEditAddress(s.address||""); setEditTransport(String(s.transport_fee||0)); setEditUnitPrice(String(s.unit_price||1200)); setEditPin(s.pin||""); setEditLicense(!!s.has_license); setEditLicenseNum(s.license_number||""); setEditOiriBonus(String(s.oiri_bonus||0)); setEditWithholding(!!s.has_withholding); setEditNightStart(s.night_start_time||"00:00"); setEditNightEnd(s.night_end_time||"05:00"); setEditNightPrice(String(s.night_unit_price||100)); setEditHasInvoice(!!s.has_invoice); setEditInvoiceNum(s.invoice_number||""); setEditAdvanceAmount(String(s.advance_preset_amount||0)); setEditInvoicePhoto(null);
                                }}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowAdd(false)}>
          <div className="rounded-2xl border p-6 w-full max-w-md max-h-[90vh] overflow-y-auto animate-[fadeIn_0.25s]" style={{ backgroundColor: T.card, borderColor: T.border }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div><label className="block text-[11px] mb-1" style={{ color: T.textSub }}>名前 *<span className="text-[9px] ml-1" style={{ color: T.textFaint }}>（源氏名OK）</span></label><input type="text" value={addName} onChange={(e) => setAddName(e.target.value)} className="w-full px-3 py-2.5 rounded-xl text-[12px] outline-none" style={inputStyle} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[11px] mb-1" style={{ color: T.textSub }}>本名<span className="text-[9px] ml-1" style={{ color: T.textFaint }}>（証明書用）</span></label><input type="text" value={addRealName} onChange={(e) => setAddRealName(e.target.value)} placeholder="山田 太郎" className="w-full px-3 py-2.5 rounded-xl text-[12px] outline-none" style={inputStyle} /></div>
                <div><label className="block text-[11px] mb-1" style={{ color: T.textSub }}>契約開始日</label><input type="date" value={addEntryDate} onChange={(e) => setAddEntryDate(e.target.value)} className="w-full px-3 py-2.5 rounded-xl text-[12px] outline-none" style={inputStyle} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
      ...
      {editStaff && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setEditStaff(null)}>
          <div className="rounded-2xl border p-6 w-full max-w-md max-h-[90vh] overflow-y-auto animate-[fadeIn_0.25s]" style={{ backgroundColor: T.card, borderColor: T.border }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div><label className="block text-[11px] mb-1" style={{ color: T.textSub }}>名前 *<span className="text-[9px] ml-1" style={{ color: T.textFaint }}>（源氏名OK）</span></label><input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full px-3 py-2.5 rounded-xl text-[12px] outline-none" style={inputStyle} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[11px] mb-1" style={{ color: T.textSub }}>本名<span className="text-[9px] ml-1" style={{ color: T.textFaint }}>（証明書用）</span></label><input type="text" value={editRealName} onChange={(e) => setEditRealName(e.target.value)} placeholder="山田 太郎" className="w-full px-3 py-2.5 rounded-xl text-[12px] outline-none" style={inputStyle} /></div>
                <div><label className="block text-[11px] mb-1" style={{ color: T.textSub }}>契約開始日</label><input type="date" value={editEntryDate} onChange={(e) => setEditEntryDate(e.target.value)} className="w-full px-3 py-2.5 rounded-xl text-[12px] outline-none" style={inputStyle} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
