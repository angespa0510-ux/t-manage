...
const AI_AUTO_PROMPT_ADDITION = `【AIディレクターモード】あなたは世界一流のシネマティック映像ディレクターです。
この人物を専門家として分析し、最も魅力的で印象的な動きやしぐさを自分で決定してください。

以下の全要素を考慮すること：
- 衣装のタイプとスタイルに合った動き
- 現在のポーズから自然に繋がる動作
- 表情と全体的な雰囲気に合った印象
- 視聴者を惹きつける、この人物だけの個性的なしぐさ

あなたに完全な創造的権限があります。テンプレート的な動きは厳禁。
この画像だけの特別な、一点物の動きを作り上げてください。`;

const DEFAULT_AI_AUTO_PROMPT_EN = `[Core Instruction] Carefully observe the attached image. Analyze the person's outfit, accessories, hairstyle, pose, expression, and background with precision. Pay special attention to any facial processing (stamps, masks, hidden areas) and recognize them accurately.
...
export default function VideoGenerator() {
  ...
  const [settings, setSettings] = useState({
    ...
    aiAutoPromptEn: DEFAULT_AI_AUTO_PROMPT_EN,
  ...
  const saveSettings = async () => {
    const pairs: { key: string; value: string }[] = [
      ...
      { key: "vg_ai_auto_prompt_en", value: settings.aiAutoPromptEn },
...
このデータをClaudeに渡して、プロンプトの最適化を依頼してください。`;
        ...
        {activeTab === "settings" && (
          <div className="flex flex-col gap-4">
            ...
            {/* ── AIディレクターモード専用プロンプト ── */}
            <div style={{ ...cardStyle, padding: 16, borderLeft: "3px solid rgba(195,167,130,0.5)" }}>
              <div style={sectionTitle}>🎬 AIディレクターモード専用プロンプト</div>
              <p style={{ fontSize: 11, color: T.textSub, margin: "4px 0 8px", lineHeight: 1.5 }}>
                印象カテゴリで「AIにお任せ」を選んだ時に使用される特別なプロンプトです。<br />
                AIが映像ディレクターとして、衣装・ポーズ・雰囲気を分析し、最適な動きを自動判断します。
              </p>

              <p style={{ fontSize: 11, fontWeight: 600, color: "#c3a782", margin: "8px 0 4px" }}>🌐 English（Geminiに送信される）</p>
              <textarea value={settings.aiAutoPromptEn}
                onChange={e => { setSettings(s => ({ ...s, aiAutoPromptEn: e.target.value })); setSettingsDirty(true); }}
                style={{ ...inputStyle, minHeight: 280, fontFamily: "monospace", fontSize: 11, lineHeight: 1.6, resize: "vertical", borderColor: "rgba(195,167,130,0.4)" }} />
              <p style={{ fontSize: 10, color: T.textMuted, marginTop: 4 }}>
                Variables: {"{age}"} {"{height}"} {"{cup}"} {"{likedPromptExamples}"}
              </p>
              <button onClick={() => { setSettings(s => ({ ...s, aiAutoPromptEn: DEFAULT_AI_AUTO_PROMPT_EN })); setSettingsDirty(true); }}
                style={{ ...btnSub, marginTop: 4, fontSize: 10 }}>🔄 英語デフォルトに戻す</button>

              <details style={{ marginTop: 12 }}>
                <summary style={{ fontSize: 11, color: T.textSub, cursor: "pointer" }}>🇯🇵 日本語（参考用）</summary>
                <pre style={{ ...inputStyle, minHeight: 120, fontFamily: "monospace", fontSize: 10, lineHeight: 1.6, marginTop: 8, opacity: 0.7, whiteSpace: "pre-wrap", padding: 10 }}>
                  {AI_AUTO_PROMPT_ADDITION}
                </pre>
              </details>
            </div>
