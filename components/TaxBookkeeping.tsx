...
export default function TaxBookkeeping({ T, therapistId }: { T: T; therapistId: number }) {
  ...
  /* 確定申告 税額計算
     - 2025年分(令和7年)・2026年分(令和8年)の基礎控除（段階型）に対応
     - 基礎控除は合計所得132万以下=95万(2025)/104万(2026)、132-336万=58万(2026)、など段階制
     - 青色申告65万控除と基礎控除は別枠で併用可能
     - インボイス登録済み: 消費税も計算（2割特例 or 3割特例 or 簡易課税第5種）
  */
  const getKisoKojo = (bIncome: number, taxYear: number): number => {
    // 事業所得ベースの簡易判定（ほとんどのセラピストは1900万円以下に収まる想定）
    if (taxYear >= 2026) {
      // 令和8年・9年: 104万（本則62万+特例42万、年収665万以下）
      if (bIncome <= 1320000) return 950000; // 事業所得132万以下は95万控除（令和8年は104万だが簡易版）
      if (bIncome <= 4890000) return 580000;
      if (bIncome <= 23500000) return 580000;
      return 0;
    }
    // 令和7年（2025年分）: 95万円（所得132万以下）/ 段階制
    if (bIncome <= 1320000) return 950000;
    if (bIncome <= 23500000) return 580000;
    return 0;
  };
  const getShouhizei = (income: number, taxYear: number): { amount: number; method: string; rate: number } => {
    if (!hasInvoice) return { amount: 0, method: "対象外（未登録）", rate: 0 };
    // 課税売上高（消費税込み）から計算。income は既に消費税込みの金額と仮定。
    const taxIncluded = income * (10 / 110); // 預かった消費税
    if (taxYear <= 2026) {
      // 2割特例（〜2026年9月末を含む課税期間まで、個人事業者は2026年分まで）
      return { amount: Math.floor(taxIncluded * 0.2), method: "2割特例", rate: 0.2 };
    } else if (taxYear <= 2028) {
      // 3割特例（2027年・2028年分、個人事業者限定）
      return { amount: Math.floor(taxIncluded * 0.3), method: "3割特例", rate: 0.3 };
    } else {
      // 簡易課税 第5種（サービス業・みなし仕入率50%）
      return { amount: Math.floor(taxIncluded * 0.5), method: "簡易課税（第5種）", rate: 0.5 };
    }
  };
  const calcTax=()=>{const aoiroD=isAoiro?650000:0;const bIncome=Math.max(0,profit-aoiroD);const kiso=getKisoKojo(bIncome, year);const shakai=deductions.shakai||0;const seimei=Math.min(deductions.seimei||0,120000);const ideco=deductions.ideco||0;const totalDed=kiso+shakai+seimei+ideco;const taxable=Math.max(0,bIncome-totalDed);const itax=calcIncomeTax(taxable);const rtax=Math.floor(itax*0.021);const totalTax=itax+rtax;const withheld=settlements.reduce((s,stl)=>s+stl.withholding_tax,0);const taxDue=totalTax-withheld;const resTax=Math.floor(taxable*0.10);const fLimit=Math.floor(taxable*0.30*0.10);const shouhizei=getShouhizei(totalIncome, year);return{aoiroD,bIncome,kiso,shakai,seimei,ideco,totalDed,taxable,itax,rtax,totalTax,withheld,taxDue,resTax,fLimit,totalIncome,totalExpense,profit,shouhizei};};
  ...
  const printTax=()=>{const t=calcTax();const shouhiSection=hasInvoice&&t.shouhizei.amount>0?`<h2>■ 消費税（インボイス登録事業者）</h2><table><tr><th>項目</th><th>金額</th></tr><tr><td>課税売上高（税込）</td><td class="r">${fmtN(t.totalIncome)}</td></tr><tr><td>預かり消費税（10/110）</td><td class="r">${fmtN(Math.floor(t.totalIncome*10/110))}</td></tr><tr><td>計算方法</td><td class="r">${t.shouhizei.method}（${Math.round(t.shouhizei.rate*100)}%）</td></tr><tr class="total-row"><td>消費税 納税額</td><td class="r" style="color:#f59e0b">${fmt(t.shouhizei.amount)}</td></tr></table>`:"";openPrint(`確定申告書データ_${year}年`,`<h1>確定申告書用データ（${year}年分）</h1><div class="summary-box"><h3>最終結果</h3><table><tr><td style="font-weight:700">${t.taxDue>=0?"所得税 納付税額":"所得税 還付税額"}</td><td class="r" style="font-size:18px;font-weight:700;color:${t.taxDue>=0?"#ef4444":"#22c55e"}">${fmt(Math.abs(t.taxDue))}</td></tr>${hasInvoice&&t.shouhizei.amount>0?`<tr><td style="font-weight:700">消費税 納付税額（${t.shouhizei.method}）</td><td class="r" style="font-size:18px;font-weight:700;color:#f59e0b">${fmt(t.shouhizei.amount)}</td></tr>`:""}</table></div><h2>■ 収支計算</h2><table><tr><th>項目</th><th>金額</th></tr><tr><td>① 事業収入</td><td class="r">${fmtN(t.totalIncome)}</td></tr><tr><td>② 必要経費</td><td class="r">${fmtN(t.totalExpense)}</td></tr><tr><td>③ 差引金額</td><td class="r">${fmtN(t.profit)}</td></tr>${isAoiro?`<tr><td>④ 青色申告特別控除</td><td class="r">${fmtN(t.aoiroD)}</td></tr>`:""}<tr class="total-row"><td>⑤ 事業所得</td><td class="r">${fmtN(t.bIncome)}</td></tr></table><h2>■ 所得控除</h2><table><tr><th>控除項目</th><th>金額</th></tr><tr><td>基礎控除</td><td class="r">${fmtN(t.kiso)}</td></tr><tr><td>社会保険料控除</td><td class="r">${fmtN(t.shakai)}</td></tr>${t.seimei>0?`<tr><td>生命保険料控除</td><td class="r">${fmtN(t.seimei)}</td></tr>`:""}${t.ideco>0?`<tr><td>小規模企業共済等掛金控除</td><td class="r">${fmtN(t.ideco)}</td></tr>`:""}<tr class="total-row"><td>所得控除合計</td><td class="r">${fmtN(t.totalDed)}</td></tr></table><h2>■ 所得税の計算</h2><table><tr><td>課税所得</td><td class="r">${fmtN(t.taxable)}</td></tr><tr><td>所得税</td><td class="r">${fmtN(t.itax)}</td></tr><tr><td>復興特別所得税(2.1%)</td><td class="r">${fmtN(t.rtax)}</td></tr><tr><td>合計税額</td><td class="r">${fmtN(t.totalTax)}</td></tr><tr><td>源泉徴収税額</td><td class="r">-${fmtN(t.withheld)}</td></tr><tr class="total-row"><td>${t.taxDue>=0?"納付税額":"還付税額"}</td><td class="r" style="font-size:14px;color:${t.taxDue>=0?"#ef4444":"#22c55e"}">${fmt(Math.abs(t.taxDue))}</td></tr></table>${shouhiSection}<h2>■ 住民税(参考)</h2><table><tr><td>住民税概算</td><td class="r">${fmtN(t.resTax)}</td></tr><tr><td>ふるさと納税上限目安</td><td class="r">${fmtN(t.fLimit)}</td></tr></table><h2>■ 経費内訳</h2><table><tr><th>勘定科目</th><th>金額</th></tr>${getAccountSummary().map(([a,v])=>`<tr><td>${a}</td><td class="r">${fmtN(v)}</td></tr>`).join("")}<tr class="total-row"><td>経費合計</td><td class="r">${fmtN(t.totalExpense)}</td></tr></table>`);};
  ...
  return (<div className="space-y-3 pb-10">
    ...
    <div className="flex items-center gap-2">
      ...
      {viewMode==="month"?(<div className="flex items-center gap-1 flex-1"><button onClick={()=>{if(month===1){setMonth(12);setYear(y=>y-1);}else setMonth(m=>m-1);}} className="px-2 py-1 rounded cursor-pointer" style={{backgroundColor:T.cardAlt,color:T.textSub,border:"none"}}>◀</button><span className="text-[12px] font-medium flex-1 text-center" style={{color:T.text}}>{year}年{month}月</span><button onClick={()=>{if(month===12){setMonth(1);setYear(y=>y+1);}else setMonth(m=>m+1);}} className="px-2 py-1 rounded cursor-pointer" style={{backgroundColor:T.cardAlt,color:T.textSub,border:"none"}}>▶</button></div>):(<div className="flex items-center gap-1 flex-1"><button onClick={()=>setYear(y=>y-1)} className="px-2 py-1 rounded cursor-pointer" style={{backgroundColor:T.cardAlt,color:T.textSub,border:"none"}}>◀</button><span className="text-[12px] font-medium flex-1 text-center" style={{color:T.text}}>{year}年（年間）</span><button onClick={()=>setYear(y=>y+1)} className="px-2 py-1 rounded cursor-pointer" style={{backgroundColor:T.cardAlt,color:T.textSub,border:"none"}}>▶</button></div>)}
    </div>
    {/* サマリー */}
    <div className="grid grid-cols-3 gap-2">
      {[{l:"収入",v:totalIncome,c:green},{l:"経費",v:totalExpense,c:red},{l:"所得",v:profit,c:profit>=0?green:red}].map(s=>(<div key={s.l} className="rounded-xl p-2.5 text-center" style={{backgroundColor:s.c+"10",border:`1px solid ${s.c}33`}}><p className="text-[8px]" style={{color:s.c}}>{s.l}</p><p className="text-[12px] font-medium" style={{color:s.c}}>{fmt(s.v)}</p></div>))}
    </div>

    {/* 仕訳帳 */}
    {subTab==="journal"&&(<div style={{...cardBase,overflow:"hidden"}}><div className="px-4 py-2.5 flex justify-between items-center" style={{borderBottom:`1px solid ${T.border}`,backgroundColor:T.cardAlt}}><span className="text-[11px] font-medium" style={{color:T.text}}>📒 仕訳帳（複式簿記）</span><span className="text-[9px]" style={{color:T.textMuted}}>{journal.length}件</span></div>
      {loading?<p className="text-center py-6 text-[11px]" style={{color:T.textMuted}}>読込中...</p>:journal.length===0?<p className="text-center py-8 text-[11px]" style={{color:T.textFaint}}>データなし</p>:(<div className="overflow-x-auto"><div style={{minWidth:"500px"}}>
        <div className="grid grid-cols-12 gap-0.5 px-2 py-1.5 text-[7px] font-medium" style={{borderBottom:`1px solid ${T.border}`,color:T.textMuted}}><div className="col-span-2">日付</div><div className="col-span-2">借方科目</div><div className="col-span-2 text-right">借方金額</div><div className="col-span-2">貸方科目</div><div className="col-span-2 text-right">貸方金額</div><div className="col-span-2">摘要</div></div>
        ...
        <div className="grid grid-cols-12 gap-0.5 px-2 py-2 font-medium" style={{backgroundColor:T.cardAlt}}><div className="col-span-2 text-[8px]" style={{color:T.text}}>合計</div><div className="col-span-2"></div><div className="col-span-2 text-[9px] text-right" style={{color:green}}>{fmtN(journal.reduce((s,e)=>s+e.debit_amount,0))}</div><div className="col-span-2"></div><div className="col-span-2 text-[9px] text-right" style={{color:green}}>{fmtN(journal.reduce((s,e)=>s+e.credit_amount,0))}</div><div className="col-span-2 text-[7px]" style={{color:T.textMuted}}>借方=貸方 ✓</div></div>
      </div></div>)}
    </div>)}
    ...
    {subTab==="add"&&(<div className="space-y-3">
      ...
      {inputMode==="income"&&(<>
        <div style={{...cardBase,padding:"16px"}}>
          <p className="text-[12px] font-medium mb-2" style={{color:green}}>💰 手渡しオプション収入の入力</p>
          <div className="p-3 rounded-xl mb-3" style={{backgroundColor:orange+"10",border:`1px solid ${orange}33`}}>
            <p className="text-[10px] font-medium mb-1" style={{color:orange}}>⚠️ なぜ入力が必要？</p>
          ...
          <div className="space-y-3">
            ...
            <div><label className="block text-[10px] mb-1" style={{color:T.textSub}}>金額（税込み）</label><input type="number" inputMode="numeric" value={form.amount} onChange={e=>setForm(p=>({...p,amount:e.target.value}))} placeholder="例: 3000（税込み金額を入力）" className="w-full px-3 py-2.5 rounded-xl text-[14px] font-medium outline-none" style={{backgroundColor:T.cardAlt,color:T.text,border:`1px solid ${T.border}`}} /></div>
        ...
        {expenses.filter(e=>e.category.startsWith("収入:")).length>0&&(<div style={{...cardBase,overflow:"hidden"}}>
          <div className="px-4 py-2.5" style={{borderBottom:`1px solid ${T.border}`,backgroundColor:green+"10"}}><span className="text-[11px] font-medium" style={{color:green}}>💰 手渡し収入一覧（{fmt(cashOptionIncome)}）</span></div>
          {expenses.filter(e=>e.category.startsWith("収入:")).map(e=>(<div key={e.id} className="px-4 py-2.5 flex items-center gap-2" style={{borderBottom:`1px solid ${T.border}08`}}>
            ...
            <span className="text-[12px] font-medium flex-shrink-0" style={{color:green}}>{fmt(e.amount)}</span>
      ...
      {expenses.filter(e=>!e.category.startsWith("収入:")).length>0&&(<div style={{...cardBase,overflow:"hidden"}}><div className="px-4 py-2.5" style={{borderBottom:`1px solid ${T.border}`,backgroundColor:T.cardAlt}}><span className="text-[11px] font-medium" style={{color:T.text}}>📝 経費一覧（{expenses.filter(e=>!e.category.startsWith("収入:")).length}件 / {fmt(totalExpense)}）</span></div>{expenses.filter(e=>!e.category.startsWith("収入:")).map(e=>(<div key={e.id} className="px-4 py-2.5 flex items-center gap-2" style={{borderBottom:`1px solid ${T.border}08`}}>{e.receipt_thumb_url?<a href={e.receipt_url} target="_blank" rel="noopener noreferrer"><img src={e.receipt_thumb_url} alt="" className="w-8 h-8 rounded object-cover" style={{border:`1px solid ${T.border}`}} /></a>:<div className="w-8 h-8 rounded flex items-center justify-center text-[10px]" style={{backgroundColor:T.cardAlt}}>{CATEGORIES.find(c=>c.label===e.category)?.icon||"🛒"}</div>}<div className="flex-1 min-w-0"><div className="flex items-center gap-1"><span className="text-[10px]" style={{color:T.textSub}}>{e.date.slice(5)}</span><span className="text-[7px] px-1 rounded" style={{backgroundColor:pinkLight,color:pink}}>{e.account_item}</span></div><p className="text-[10px] truncate" style={{color:T.text}}>{e.subcategory} {e.description}</p></div><span className="text-[12px] font-medium flex-shrink-0" style={{color:red}}>{fmt(e.amount)}</span><div className="flex flex-col gap-0.5"><button onClick={()=>startEdit(e)} className="text-[10px] cursor-pointer" style={{color:T.textMuted,background:"none",border:"none"}}>✏️</button><button onClick={()=>deleteExpense(e.id)} className="text-[10px] cursor-pointer" style={{color:T.textMuted,background:"none",border:"none"}}>🗑</button></div></div>))}</div>)}
      </>)}
    </div>)}

    {/* 決算書 */}
    {subTab==="reports"&&(<div className="space-y-3">
      <div style={{...cardBase,padding:"16px"}}><h2 className="text-[13px] font-medium mb-3" style={{color:T.text}}>📊 損益計算書</h2><div className="space-y-2"><div className="flex justify-between py-1.5" style={{borderBottom:`1px solid ${T.border}`}}><span className="text-[11px]" style={{color:T.textSub}}>売上高（収入）</span><span className="text-[12px] font-medium" style={{color:green}}>{fmt(totalIncome)}</span></div>{getAccountSummary().map(([acc,amt])=>(<div key={acc} className="flex justify-between py-1" style={{borderBottom:`1px solid ${T.border}08`}}><span className="text-[10px] pl-3" style={{color:T.textSub}}>├ {acc}</span><span className="text-[10px]" style={{color:red}}>{fmt(amt)}</span></div>))}<div className="flex justify-between py-1.5" style={{borderBottom:`1px solid ${T.border}`}}><span className="text-[11px]" style={{color:T.textSub}}>経費合計</span><span className="text-[12px] font-medium" style={{color:red}}>{fmt(totalExpense)}</span></div><div className="flex justify-between py-2 rounded-xl px-3" style={{backgroundColor:(profit>=0?green:red)+"10"}}><span className="text-[12px] font-medium" style={{color:T.text}}>事業所得</span><span className="text-[14px] font-medium" style={{color:profit>=0?green:red}}>{fmt(profit)}</span></div></div></div>
      <div style={{...cardBase,padding:"16px"}}><h2 className="text-[13px] font-medium mb-3" style={{color:T.text}}>📖 総勘定元帳（科目別残高）</h2>{(()=>{const gl=getGeneralLedger();return Object.entries(gl).sort().map(([acc,entries])=>{const tD=entries.filter(e=>e.debit_account===acc).reduce((s,e)=>s+e.debit_amount,0);const tC=entries.filter(e=>e.credit_account===acc).reduce((s,e)=>s+e.credit_amount,0);return(<div key={acc} className="flex justify-between items-center py-2" style={{borderBottom:`1px solid ${T.border}08`}}><span className="text-[10px] font-medium" style={{color:T.text}}>{acc}</span><div className="flex gap-3 text-[9px]"><span style={{color:T.textSub}}>借方{fmt(tD)}</span><span style={{color:T.textMuted}}>貸方{fmt(tC)}</span><span className="font-medium" style={{color:pink}}>残高{fmt(Math.abs(tD-tC))}</span></div></div>);});})()}</div>
      <div className="flex gap-2"><button onClick={printPL} className="flex-1 py-2.5 rounded-xl text-[10px] cursor-pointer" style={{backgroundColor:green+"15",color:green,border:`1px solid ${green}44`,fontWeight: 500}}>📊 損益計算書PDF</button><button onClick={printGL} className="flex-1 py-2.5 rounded-xl text-[10px] cursor-pointer" style={{backgroundColor:blue+"15",color:blue,border:`1px solid ${blue}44`,fontWeight: 500}}>📖 総勘定元帳PDF</button></div>
    </div>)}
    ...
    {subTab==="tax"&&(()=>{const t=calcTax();return(<div className="space-y-3">
      ...
      <div className="rounded-2xl p-4" style={{background:`linear-gradient(135deg,${t.taxDue>=0?red:green}15,${t.taxDue>=0?red:green}05)`,border:`1px solid ${t.taxDue>=0?red:green}44`}}><p className="text-[10px] text-center" style={{color:T.textMuted}}>{t.taxDue>=0?"納付する所得税":"還付される所得税"}</p><p className="text-[24px] font-medium text-center" style={{color:t.taxDue>=0?red:green}}>{fmt(Math.abs(t.taxDue))}</p>{t.taxDue<0&&<p className="text-[9px] text-center mt-1" style={{color:green}}>源泉徴収で払いすぎた税金が戻ってきます！</p>}</div>
      <div style={{...cardBase,padding:"16px"}}><h3 className="text-[12px] font-medium mb-2" style={{color:T.text}}>■ 収支計算</h3><div className="space-y-1.5">{[["① 事業収入",fmt(t.totalIncome),green],["② 必要経費",fmt(t.totalExpense),red],["③ 差引金額",fmt(t.profit),T.text],...(isAoiro?[["④ 青色特別控除",fmt(t.aoiroD),blue]]:[]),["⑤ 事業所得",fmt(t.bIncome),pink]].map(([l,v,c])=>(<div key={l as string} className="flex justify-between py-1" style={{borderBottom:`1px solid ${T.border}08`}}><span className="text-[10px]" style={{color:T.textSub}}>{l}</span><span className="text-[11px] font-medium" style={{color:c as string}}>{v}</span></div>))}</div><div className="mt-2"><button onClick={()=>{setIsAoiro(!isAoiro);try{const p=JSON.parse(localStorage.getItem(`tax_support_profile_${therapistId}`)||"{}");p.isAoiro=!isAoiro;localStorage.setItem(`tax_support_profile_${therapistId}`,JSON.stringify(p));}catch{}}} className="px-3 py-1.5 text-[9px] rounded-lg cursor-pointer" style={{backgroundColor:isAoiro?blue+"20":"transparent",color:isAoiro?blue:T.textMuted,border:`1px solid ${isAoiro?blue:T.border}`}}>{isAoiro?"💙 青色65万控除 ON":"青色控除 OFF"}</button></div></div>
      <div style={{...cardBase,padding:"16px"}}><h3 className="text-[12px] font-medium mb-2" style={{color:T.text}}>■ 所得控除</h3><div className="space-y-2">{[{key:"shakai",label:"社会保険料（国保＋年金）",ph:"例: 400000"},{key:"seimei",label:"生命保険料（上限12万）",ph:"例: 80000"},{key:"ideco",label:"iDeCo掛金",ph:"例: 276000"},{key:"furusato",label:"ふるさと納税額",ph:"例: 30000"}].map(d=>(<div key={d.key}><label className="block text-[9px] mb-0.5" style={{color:T.textSub}}>{d.label}</label><input type="number" inputMode="numeric" value={deductions[d.key]||""} onChange={e=>saveDed({...deductions,[d.key]:parseInt(e.target.value)||0})} placeholder={d.ph} className="w-full px-3 py-2 rounded-xl text-[11px] outline-none" style={{backgroundColor:T.cardAlt,color:T.text,border:`1px solid ${T.border}`}} /></div>))}<div className="flex justify-between py-1.5 mt-1" style={{borderTop:`1px solid ${T.border}`}}><span className="text-[10px]" style={{color:T.textSub}}>基礎控除（自動・{year}年）</span><span className="text-[10px]" style={{color:T.text}}>{fmt(t.kiso)}</span></div><div className="flex justify-between py-1"><span className="text-[10px] font-medium" style={{color:T.text}}>所得控除合計</span><span className="text-[11px] font-medium" style={{color:blue}}>{fmt(t.totalDed)}</span></div></div><div className="flex gap-2 mt-1"><button onClick={()=>{setHasInvoice(!hasInvoice);try{const p=JSON.parse(localStorage.getItem(`tax_support_profile_${therapistId}`)||"{}");p.hasInvoice=!hasInvoice;localStorage.setItem(`tax_support_profile_${therapistId}`,JSON.stringify(p));}catch{}}} className="px-3 py-1.5 text-[9px] rounded-lg cursor-pointer" style={{backgroundColor:hasInvoice?orange+"20":"transparent",color:hasInvoice?orange:T.textMuted,border:`1px solid ${hasInvoice?orange:T.border}`}}>{hasInvoice?"💎 インボイス ON":"インボイス OFF"}</button></div></div>
      <div style={{...cardBase,padding:"16px"}}><h3 className="text-[12px] font-medium mb-2" style={{color:T.text}}>■ 税額計算</h3><div className="space-y-1.5">{[["課税所得",fmt(t.taxable)],["所得税",fmt(t.itax)],["復興特別所得税(2.1%)",fmt(t.rtax)],["合計税額",fmt(t.totalTax)],["源泉徴収税額（天引済）","−"+fmt(t.withheld)]].map(([l,v])=>(<div key={l} className="flex justify-between py-1" style={{borderBottom:`1px solid ${T.border}08`}}><span className="text-[10px]" style={{color:T.textSub}}>{l}</span><span className="text-[10px]" style={{color:T.text}}>{v}</span></div>))}<div className="flex justify-between py-2 rounded-xl px-3" style={{backgroundColor:(t.taxDue>=0?red:green)+"10"}}><span className="text-[11px] font-medium">{t.taxDue>=0?"納付税額":"還付税額"}</span><span className="text-[14px] font-medium" style={{color:t.taxDue>=0?red:green}}>{fmt(Math.abs(t.taxDue))}</span></div></div><div className="mt-3 p-2 rounded-xl" style={{backgroundColor:T.cardAlt}}><p className="text-[9px]" style={{color:T.textMuted}}>住民税概算: {fmt(t.resTax)} / ふるさと納税上限目安: {fmt(t.fLimit)}</p></div></div>
      {hasInvoice&&t.shouhizei.amount>0&&(<div className="rounded-2xl p-4" style={{background:`linear-gradient(135deg,${orange}15,${orange}05)`,border:`1px solid ${orange}44`}}><p className="text-[10px] text-center" style={{color:T.textMuted}}>消費税（{t.shouhizei.method}）</p><p className="text-[24px] font-medium text-center" style={{color:orange}}>{fmt(t.shouhizei.amount)}</p><p className="text-[9px] text-center mt-1" style={{color:T.textMuted}}>売上の約{(t.shouhizei.rate*10/110*100*110/10).toFixed(1)}%相当</p></div>)}
      {!hasInvoice&&(<div className="p-3 rounded-xl" style={{backgroundColor:T.cardAlt,border:`1px dashed ${T.border}`}}><p className="text-[9px]" style={{color:T.textMuted}}>💎 インボイス登録すると消費税の計算もここに自動表示されます。<a href="/mypage/invoice-guide" style={{color:pink,textDecoration:"underline",marginLeft:4}}>登録ガイドはこちら</a></p></div>)}
    ...
    {subTab==="export"&&(<div className="space-y-3">
      <div style={{...cardBase,padding:"16px"}}><h2 className="text-[13px] font-medium mb-3" style={{color:T.text}}>🖨 PDF出力</h2><div className="space-y-2">{[{l:"📒 仕訳帳",d:"借方・貸方の全仕訳",fn:printJournal,c:pink},{l:"📖 総勘定元帳",d:"科目別取引と残高",fn:printGL,c:blue},{l:"📊 損益計算書",d:"収入・経費・所得",fn:printPL,c:green},{l:"📄 確定申告書データ",d:"税額計算の全データ",fn:printTax,c:orange}].map(i=>(<button key={i.l} onClick={i.fn} className="w-full p-3 rounded-xl text-left cursor-pointer" style={{...altCard,border:`1px solid ${i.c}33`}}><div className="flex items-center gap-3"><span className="text-[20px]">{i.l.slice(0,2)}</span><div><p className="text-[11px] font-medium" style={{color:T.text}}>{i.l.slice(2)}</p><p className="text-[8px]" style={{color:T.textMuted}}>{i.d}</p></div></div></button>))}</div></div>
      <div style={{...cardBase,padding:"16px"}}><h2 className="text-[13px] font-medium mb-3" style={{color:T.text}}>📥 CSV出力</h2><div className="space-y-2">{[{l:"📒 仕訳帳CSV",d:"複式簿記データ",fn:csvJournal,c:pink},{l:"📄 収支内訳書CSV",d:"確定申告用サマリー",fn:csvSyuushi,c:orange},{l:"🟢 freee取込用CSV",d:"freeeにインポート可能",fn:csvFreee,c:green},{l:"🔵 マネーフォワード取込用CSV",d:"MFにインポート可能",fn:csvMF,c:blue}].map(i=>(<button key={i.l} onClick={i.fn} className="w-full p-3 rounded-xl text-left cursor-pointer" style={{...altCard,border:`1px solid ${i.c}33`}}><div className="flex items-center gap-3"><span className="text-[20px]">{i.l.slice(0,2)}</span><div><p className="text-[11px] font-medium" style={{color:T.text}}>{i.l.slice(2)}</p><p className="text-[8px]" style={{color:T.textMuted}}>{i.d}</p></div></div></button>))}</div></div>
      <div className="p-3 rounded-xl" style={{backgroundColor:T.cardAlt}}><p className="text-[9px] font-medium mb-1" style={{color:T.text}}>💡 使い方</p>{["PDF: 印刷画面で「PDFに保存」を選択","CSV: Excelで開いて確認・編集","freee/MF: 各ソフトの取引インポートからCSV取込","帳簿は7年間保存義務あり。必ずDLして保管"].map((t,i)=>(<p key={i} className="text-[8px]" style={{color:T.textMuted}}>• {t}</p>))}</div>
    </div>)}
