...
• 経費もこの口座から支払い`,
    ...
    {
      q: "🏪 掛け持ち（複数のお店）で働く場合は？",
      a: `複数のお店で業務委託として働いている場合のポイントをまとめます。

━━━ 確定申告はどうなる？ ━━━
✅ 全てのお店からの収入を合算して1つの確定申告書で申告します。
• お店ごとに別々に申告するのではなく、1年分の「全収入」をまとめて申告
• 各お店から「支払調書」をもらえれば収入把握が楽に
• T-MANAGEの帳簿には当店の清算データが自動反映されますが、他店の収入は手動で追加してください

━━━ 支払調書について ━━━
• 各お店に「支払調書を出してほしい」とお願いしましょう（1月〜2月頃）
• 支払調書は法律上の交付義務はありませんが、多くのお店が対応してくれます
• もらえない場合は、自分の給料明細や振込記録で代用可能

━━━ 経費の按分（あんぶん） ━━━
複数のお店で使う経費は「按分」が必要です：
• 交通費 → お店ごとに分けて記録するのがベスト
• 衣装・備品 → どのお店でも使うなら全額経費OK
• 携帯電話 → 仕事で使う割合を決めて計上（例：仕事70%）
• 按分の根拠（出勤日数の比率など）をメモしておくと安心

━━━ 扶養・社会保険への影響 ━━━
⚠️ 掛け持ちの場合は「全てのお店の合計収入」で判定されます：
• 年間所得48万円超 → 配偶者控除から外れる可能性
• 年間収入130万円超 → 社会保険の扶養から外れる可能性
• 家族の扶養に入っている方は要注意！

━━━ 本業がある場合の注意 ━━━
• 住民税の「普通徴収」は全ての副業収入に対して選択してください
• 掛け持ち先が全て業務委託なら、確定申告で一括処理できます
• 1ヶ所でも「雇用契約（パート・アルバイト）」がある場合は、年末調整と確定申告の両方が必要になることがあります

━━━ 帳簿のつけ方 ━━━
✅ お店ごとに収入を分けて記録すると管理が楽です：
• T-MANAGEの帳簿 → 当店の収入は自動記録
• 他店の収入 → 「その他収入」として手動で追加
• 摘要欄に「○○店 業務委託報酬」などと記載すると分かりやすい`,
    },
  ];
